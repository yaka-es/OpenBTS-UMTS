...
RadioInterface::RadioInterface(RadioDevice *wRadio, int wReceiveOffset, UMTS::Time wStartTime)
	: mRadio(wRadio), sendCursor(0), recvCursor(0), underrun(false), receiveOffset(wReceiveOffset), mOn(false),
	  powerScaling(TX_BASE_SCALING), finalVec(NULL), upsampler(NULL), dnsampler(NULL), innerSendBuffer(NULL),
	  innerRecvBuffer(NULL), outerSendBuffer(NULL), outerRecvBuffer(NULL), convertSendBuffer(NULL),
...
bool RadioInterface::init()
{
	...
	if (!dnsampler->init(Resampler::FILTER_TYPE_RRC, (float)RESAMP_INRATE / (float)RESAMP_OUTRATE)) {
...
void RadioInterface::unRadioifyVector(float *floatVector, signalVector &newVector)
{
	memcpy(newVector.begin(), floatVector, newVector.size() * sizeof(Complex<float>));
}
...
bool RadioInterface::pushBuffer(void)
{
	...
	convert_float_short(convertSendBuffer, (float *)outerSendBuffer->begin(), powerScaling, outer_len * 2);

	mRadio->writeSamples(convertSendBuffer, outer_len, &underrun, writeTimestamp);

	/* Shift remaining samples to beginning of buffer */
	memmove(innerSendBuffer->begin() + upsampler->len(), innerSendBuffer->begin() + upsampler->len() + inner_len,
...
bool RadioInterface::pullBuffer(void)
{
	...
	size_t num_recv = mRadio->readSamples(convertRecvBuffer, outchunk, &overrun, readTimestamp, &localUnderrun);
	...
	int rc = dnsampler->rotate(resamp_in, outchunk, resamp_out, inchunk);
	if (rc < 0) {
...
void RadioInterface::driveTransmitRadio(signalVector &radioBurst, bool zeroBurst)
{
	...
	float *pos = (float *)(innerSendBuffer->begin() + upsampler->len() + sendCursor);
...
void RadioInterface::driveReceiveRadio(int guardPeriod)
{
	...
	while (recvSz > vecSz) {
		...
		if (recvClock.FN() >= 0) {
			...
			memcpy(rxBurst->begin(), innerRecvBuffer->begin() + readSz,
	...
	memmove(innerRecvBuffer->begin(), innerRecvBuffer->begin() + readSz,
