...
PowerManager::PowerManager()
	: mAveragedT3122(gConfig.getNum("UMTS.Radio.PowerManager.TargetT3122")),
	  mAtten(gConfig.getNum("UMTS.Radio.PowerManager.MaxAttenDB")), mNextSampleIndex(0)
{
...
void PowerManager::serviceLoop()
{
	while (1) {
		...
		sampleT3122();
		if (mLast.elapsed() < gConfig.getNum("UMTS.Radio.PowerManager.Period"))
			continue;
		internalControlStep();
		mLast.now();
	}
