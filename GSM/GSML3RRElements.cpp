...
void L3FrequencyList::writeV(L3Frame &dest, size_t &wp) const
{
	...
	if (numBits < delta) {
		LOG(ALERT) << "L3FrequencyList cannot encode full ARFCN set";
	}
...
void L3RRCause::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mCauseValue, 8); }

void L3RRCause::parseV(const L3Frame &src, size_t &rp) { mCauseValue = src.readField(rp, 8); }

void L3RRCause::text(ostream &os) const { os << "0x" << hex << mCauseValue << dec; }
...
L3APDUData::L3APDUData(BitVector data) : L3ProtocolElement(), mData(data) {}
...
L3SI3RestOctets::L3SI3RestOctets()
	: L3RestOctets(), mHaveSelectionParameters(false), mCBQ(0), mCELL_RESELECT_OFFSET(0), mTEMPORARY_OFFSET(0),
	  mPENALTY_TIME(0)
{
