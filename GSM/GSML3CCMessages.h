...
public:
	...
	L3Release(unsigned wTI, const L3Cause &wCause) : L3CCMessage(wTI), mHaveCause(true), mCause(wCause) {}
...
public:
	...
	L3ReleaseComplete(unsigned wTI, const L3Cause &wCause) : L3CCMessage(wTI), mHaveCause(true), mCause(wCause) {}
...
public:
	L3Setup(unsigned wTI = 7)
		: L3CCMessage(wTI), mHaveBearerCapability(false), mHaveCallingPartyBCDNumber(false),
	...
	L3Setup(unsigned wTI, const L3CallingPartyBCDNumber &wCallingPartyBCDNumber)
		: L3CCMessage(wTI), mHaveBearerCapability(false), mHaveCallingPartyBCDNumber(true),
		  mCallingPartyBCDNumber(wCallingPartyBCDNumber), mHaveCalledPartyBCDNumber(false)
	{
	}
...
public:
	L3CallProceeding(unsigned wTI = 7) : L3CCMessage(wTI), mHaveBearerCapability(false), mHaveProgress(false) {}
...
public:
	L3Alerting(unsigned wTI = 7) : L3CCMessage(wTI), mHaveProgress(false) {}
...
public:
	...
	L3Disconnect(unsigned wTI = 7, const L3Cause &wCause = L3Cause(0x10)) : L3CCMessage(wTI), mCause(wCause) {}
...
public:
	L3CallConfirmed(unsigned wTI = 7) : L3CCMessage(wTI), mHaveCause(false) {}
...
public:
	...
	L3StartDTMFReject(unsigned wTI, const L3Cause &wCause) : L3CCMessage(wTI), mCause(wCause) {}
...
public:
	/** Reject with default cause 0x3f, "service or option not available". */
	L3HoldReject(unsigned wTI, const L3Cause &wCause) : L3CCMessage(wTI), mCause(wCause) {}
