...
L2Frame::L2Frame(const BitVector &bits, Primitive prim) : BitVector(23 * 8), mPrimitive(prim)
{
	idleFill();
...
L2Frame::L2Frame(const L2Header &header, const BitVector &l3) : BitVector(23 * 8), mPrimitive(DATA)
{
	idleFill();
...
L2Frame::L2Frame(const L2Header &header) : BitVector(23 * 8), mPrimitive(DATA)
{
	idleFill();
	header.write(*this);
}
...
L2Control::FrameType L2Frame::UFrameType() const
{
	...
	case 0x03:
		return L2Control::DMFrame;
	case 0x00:
		return L2Control::UIFrame;
	case 0x08:
		return L2Control::DISCFrame;
	case 0x0c:
		return L2Control::UAFrame;
	default:
...
L3Frame::L3Frame(const L3Message &msg, Primitive wPrimitive)
	: BitVector(msg.bitsNeeded()), mPrimitive(wPrimitive), mL2Length(msg.L2Length())
{
	msg.write(*this);
}

L3Frame::L3Frame(const char *hexString) : mPrimitive(DATA)
{
...
L3Frame::L3Frame(const char *binary, size_t len) : mPrimitive(DATA)
{
	mL2Length = len;
	resize(len * 8);
	size_t wp = 0;
