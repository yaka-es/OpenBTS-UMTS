...
void L3CMServiceType::parseV(const L3Frame &src, size_t &rp) { mType = (TypeCode)src.readField(rp, 4); }
...
void L3RejectCause::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mRejectCause, 8); }

void L3RejectCause::text(ostream &os) const { os << "0x" << hex << mRejectCause << dec; }
