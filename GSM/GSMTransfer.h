...
public:
	...
	L2Frame(GSM::Primitive wPrimitive = UNIT_DATA) : BitVector(23 * 8), mPrimitive(wPrimitive) { idleFill(); }
...
public:
	...
	L3Frame(const L3Frame &f1, const L3Frame &f2)
		: BitVector(f1, f2), mPrimitive(DATA), mL2Length(f1.mL2Length + f2.mL2Length)
	{
	}
...
public:
	VocoderFrame() : BitVector(264) { fillField(0, 0x0d, 4); }

	/** Construct by unpacking a char[33]. */
	VocoderFrame(const unsigned char *src) : BitVector(264) { unpack(src); }
