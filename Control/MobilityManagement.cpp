...
bool authenticateViaCaching(const char *IMSI, UMTS::DCCHLogicalChannel *DCCH)
{
	...
	if (!uRAND) {
		...
		uRAND = random();
		uRAND = (uRAND << 32) + random();
		lRAND = random();
		lRAND = (lRAND << 32) + random();
		// Do the transaction.
...
*/
bool sendWelcomeMessage(const char *messageName, const char *shortCodeName, const char *IMSI,
	UMTS::DCCHLogicalChannel *DCCH, const char *whiteListCode = NULL)
{
	if (!gConfig.defines(messageName))
		return false;
	...
	deliverSMSToMS(gConfig.getStr(shortCodeName).c_str(), message.str().c_str(), "text/plain", random() % 7, DCCH);
	return true;
}
...
void Control::LocationUpdatingController(const GSM::L3LocationUpdatingRequest *lur, UMTS::DCCHLogicalChannel *DCCH)
{
	...
	if (RAND.length() != 0) {
		...
		try {
			...
			LOG(ALERT)
			"SIP authentication timed out.  Is the proxy running at "
