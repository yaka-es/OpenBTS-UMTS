...
void SIPInterface::drive()
{
	...
	try {
		...
		if (msg->sip_method)
			LOG(DEBUG) << "read method " << msg->sip_method;
...
bool SIPInterface::checkInvite(osip_message_t *msg)
{
	...
	if (serviceType == GSM::L3CMServiceType::MobileTerminatedShortMessage) {
		...
		if (text)
			...
			LOG(NOTICE) << "MTSMS incoming MESSAGE method with no message body for " << mobileID;
		...
		*/
		...
		if (type) {
			...
			LOG(NOTICE) << "MTSMS incoming MESSAGE method with no content type (or memory error) for "
				    << mobileID;
	}
