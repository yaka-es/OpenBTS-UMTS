...
#include <SGSNGGSN/GPRSL3Messages.h> // For SmQoS
...
void testRlcRecv(URlcUpSdu &sdu, RbId rbid)
{
	...
	if (sdu != *expect) {
		...
		printf("RLC test vector %d did not match: expected=%s got=%s\n", sTestRecvVectorNum,
			sdu.hexstr().c_str(), expect->hexstr().c_str());
	} else {
...
int rlcTest(int argc, char **argv, ostream &os)
{
	...
	while (argi < argc) {
		...
		help:
			printf("rlctest -am|-tm|-um -s -d -ps -n <numvectors> -loss <percentloss> -seed <randomseed> "
			       "-reset[12] <pdunum>\n");
			printf("note: -ps = packet-switched-config -d = debug; -s = lossless transmission for "
			       "status\n");
			return 0;
		}
	}
	...
	for (unsigned n = 0; n < sNumTestVectors; n++) {
		...
		if (reset1 && (int)n == reset1) {
			trans1->triggerReset();
		}
		if (reset2 && (int)n == reset2) {
			trans2->triggerReset();
		}
	}
	/***
...
int rrcTest(int argc, char **argv, ostream &os)
{
	...
	if (0 == strcmp(subcmd, "rbrelease")) {
		...
		ASN::UL_CCCH_Message *msg1 =
			(ASN::UL_CCCH_Message *)uperDecodeFromByteV(&ASN::asn_DEF_UL_CCCH_Message, pdu);
