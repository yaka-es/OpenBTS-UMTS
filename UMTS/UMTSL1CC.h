...
#include <CommonLibs/TurboCoder.h>
...
// The programming for the above is horrendous, and is encapsulated by L1TrChProgInfo and related L1FecProgInfo and
// L1TrChProgInfo. Setting up a CCTrCh involves these steps: A DCHFEC (or other FEC) is a thin wrapper over an L1CCTrCh.
...
// When someone wants to use the DCHFEC, the RRC specifies the TrCh configuration, which may be different for CS or PS
// services. The RRC TrChConfig is used first to program the L1TrChProgInfo, then that is applied to the L1CCTrCh to
// allocate all the encoder/decoder classes.
...
public:
	...
	TfIndex mTfi; // The index into the mEncoders or mDecoders array.
		      // For downlink there is only one encoder per TF, so each TFC mapping to that TF has the same
		      // mTfi. For uplink, currently mTfi == TFC index, but when/if we start allowing multiple different
		      // TTI in the same CCTrCh, and if the uplink multiple radio frame aggregation buffer is in the
...
public:
	unsigned mNumTfc;
	unsigned mNumTrCh;
	unsigned l1GetNumRadioFrames(TrChId tcid) { return TTICode2NumFrames(mPerTrCh[tcid].mTTICode); }
	...
	bool fecConfigForOneTrCh(bool isDownlink, unsigned wSF, TTICodes wTTICode, unsigned wPB, unsigned wRadioFrameSz,
		unsigned wTBSz, unsigned wMinTB, unsigned wMaxTB, bool wTurbo);
};
...
public:
	L1TrChEncoderTurbo(L1CCTrCh *wParent, L1FecProgInfo *wfpi)
		: L1TrChEncoder(wParent, wfpi), mInterleaver(wfpi->mCodeInBkSz)
	{
	}
...
public:
	L1TrChDecoderTurbo(L1CCTrCh *wParent, L1FecProgInfo *wfpi)
		: L1TrChDecoder(wParent, wfpi), mInterleaver(wfpi->mCodeInBkSz)
	{
	}
...
public:
	void l1WriteLowSideFrame(const RxBitsBurst &burst, float tfci[30]);

private:
	SoftVector mDSlotAccumulatorBuf; // uplink data in
protected:
	void l1AccumulateSlots(const SoftVector *e, const float tfcibits[2]);

private:
	SoftVector mHDIBuf; // uplink 2nd De-interleaving buffer.
protected:
	void l1SecondDeinterleaving(SoftVector &e, unsigned tfci, unsigned frameIndex);

protected:
	void l1Demultiplexer(SoftVector &e, unsigned tfci, unsigned frameIndex);

protected:
	SoftVector mFillerBurst;

public:
	SoftVector *l1FillerBurst(unsigned size);
	...
	MacEngine *l1SetUpstream(MacEngine *wUpstream, TrChId tcid = 0)
	{
		...
		for (TfcId j = 0; j < RrcDefs::maxTfc; j++) {
			if (mDecoders[tcid][j]) {
				mDecoders[tcid][j]->l1SetUpstream(wUpstream);
			}
		}
		return NULL;
	}
...
public:
	...
	unsigned l1GetDlTrBkSz()
	{ // Used for simple channels with 1 TrCh, not TFCS.
...
public:
	L1ControlInterface() : mAssignmentTimer(10000), mReleaseTimer(10000)
	{
...
class L1CCTrCh : public virtual L1CCBase, // Construct this before others.
