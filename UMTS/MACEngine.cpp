...
{
	if (mDownstream)
		return mDownstream->l1GetDlNumRadioFrames(0);
	if (mccDownstream)
		return mccDownstream->l1GetDlNumRadioFrames(0);
	assert(0);
}
void MacEngine::macWriteHighSide(MacTbDl &tb)
{
	if (mDownstream)
		mDownstream->l1WriteHighSide(tb);
	if (mccDownstream)
		mccDownstream->l1WriteHighSide(tb);
}

unsigned MacEngine::macGetDlTrBkSz()
{
	if (mDownstream)
		return mDownstream->l1GetDlTrBkSz();
	if (mccDownstream)
		return mccDownstream->l1GetDlTrBkSz();
	assert(0);
}
...
void macUnHookupDch(UEInfo *uep)
{
	...
	if (mac) {
		gMacSwitch.rmMac(mac);
	} // stops downstream communcation.
	if (dch) {
...
MaccTbDlCcch::MaccTbDlCcch(unsigned trbksize, ByteVector *pdu) : MacTbDl(trbksize)
{
...
MaccTbDl::MaccTbDl(unsigned trbksize, ByteVector *pdu, UEInfo *uep, RbId rbid) : MacTbDl(trbksize)
{
...
void MacSwitch::addMac(MacEngine *mac, bool useForCcch)
{
	...
	if (useForCcch) {
		mCchList.push_back(dynamic_cast<MaccBase *>(mac));
	}
...
bool MacWithTfc::matchTfc(RrcTfc *tfc, UEInfo *uep, TfcMap *map)
{
	...
	RN_UE_FOR_ALL_DLTRCH(uep, tcid)
	{
		...
		if (map->getNumTbAvail(tcid) < numTB) {
			return false;
		}
...
bool MaccSimple::flushUE()
{
	...
	{
		if (uep->ueGetState() != stCELL_FACH) {
			continue;
		}
		if (gMacSwitch.pickFachMac(uep->mURNTI) != this) {
			continue;
		}
...
bool MaccWithTfc::flushUE()
{
	...
	if (chosenUE == 0)
		return false; // Nothing to send anywhere.
	if (chosenUE->ueGetState() != stCELL_FACH) {
		return false;
	} // in case user switched states during above loop
