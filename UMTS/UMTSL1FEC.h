 ...
 * A map of channels in the ARFCNManager give the PhCH processors access to PhCH parameters for decoding recevied
bursts.
*/
...
struct FecProgInfoInit : public FecProgInfo {
	FecProgInfoInit(unsigned wSF, TTICodes wTTICode, unsigned wPB, unsigned wRadioFrameSz, unsigned wTBSz,
		bool wTurbo = false);
};
...
public:
	TrCHFECBase(TrCHFEC *wParent, FecProgInfo &fpi) : FecProgInfo(fpi), mParent(wParent) //, mConsts(fpi.mTTICode)
	{
	}
...
public:
	...
	*/
	...
	{
		if (mDecoder)
			return mDecoder->l1SetUpstream(mux, testmode);
		else
			return 0;
	}
	...
	{
		assert(mEncoder);
		mEncoder->l1WaitToSend();
	}

	UMTS::Time nextWriteTime() const
	{
		assert(mEncoder);
		return mEncoder->nextWriteTime();
	}
	...
	{
		assert(mEncoder);
		return mEncoder->trBkSz();
	}
	unsigned l1GetUlTrBkSz() const
	{
		assert(mDecoder);
		return mDecoder->trBkSz();
	}

	unsigned l1GetDlNumRadioFrames(TrChId tcid) const
	{
		assert(tcid == 0);
		return mEncoder->getNumRadioFrames();
	}
...
public:
	TrCHFECEncoderTurbo(TrCHFEC *wParent, FecProgInfo &fpi)
		: TrCHFECEncoder(wParent, fpi), mInterleaver(fpi.mCodeBkSz)
	{
	}
...
public:
	TrCHFECDecoderTurbo(TrCHFEC *wParent, FecProgInfo &fpi)
		: TrCHFECDecoder(wParent, fpi), mInterleaver(fpi.mCodeBkSz)
	{
	}
	void decode(const SoftVector &c, BitVector &o);
	unsigned getZ() const { return 5114; } // Max Turbo encoder block size is a constant from 25.212 4.2.3
...
public:
	...
	PCHFEC(unsigned wSF, unsigned wSpCode, ARFCNManager *wRadio)
		: PhChDownlink(SCCPCHType, wSF, wSpCode, wRadio), TrCHFEC(this)
	{
...
class FACHFEC : public PhChDownlink,
		public L1CCTrCh // L1FEC_t
{
public:
	// (pat) Downlink channels do not get a scrambling code.
	FACHFEC(unsigned wSF, unsigned wSpCode, unsigned wPB __attribute_used__, unsigned wTBSize __attribute_used__,
		TTICodes tti __attribute_used__, ARFCNManager *wRadio)
		: PhChDownlink(SCCPCHType, wSF, wSpCode, wRadio), L1CCTrCh(this) // L1FEC_t(this)
	{
#if USE_OLD_FEC
...
class RACHFEC : public PhChUplink,
		public L1CCTrCh // L1FEC_t
{
public:
	RACHFEC(unsigned wSF, unsigned wUplinkScramblingCode, unsigned wPB __attribute_used__,
		unsigned wTBSize __attribute_used__, TTICodes tti __attribute_used__)
		: PhChUplink(PRACHType, wSF, wUplinkScramblingCode), L1CCTrCh(this)
...
class DCHFEC : public PhCh,
	       public TrCHFEC // single TB, single TFC.
{
public:
	DCHFEC(unsigned wDlSF, unsigned wSpCode, unsigned wUlSF, unsigned wSrCode, ARFCNManager *wRadio)
		: PhCh(DPDCHType, wDlSF, wSpCode, wUlSF, wSrCode, wRadio), TrCHFEC(this)
	{
	}
...
#else
class DCHFEC : public PhCh,
	       public L1CCTrCh // multiple TFC, multiple TB
{
public:
	DCHFEC(unsigned wDlSF, unsigned wSpCode, unsigned wUlSF, unsigned wSrCode, ARFCNManager *wRadio)
		: PhCh(DPDCHType, wDlSF, wSpCode, wUlSF, wSrCode, wRadio), L1CCTrCh(this)
	{
	}
	void open();
	void close();
