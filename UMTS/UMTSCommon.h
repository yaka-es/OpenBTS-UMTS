...
public:
	Time(uint16_t wFN = 0, unsigned wTN = 0) : mFN(wFN % gHyperframe), mTN(wTN) {}

	Time slot(unsigned s) const
	{
		assert(s < gFrameSlots);
		return Time(mFN, s);
	}
...
public:
	Clock(const UMTS::Time &when = UMTS::Time(0)) : mBaseFN(when.FN()) {}
...
enum PhyChanBranch { I_ONLY = 0, Q_ONLY = 1, I_AND_Q = 2 };
...
public:
	PhyChanParams(TrCHFEC *wHighSideFEC, UMTS::Time wStartTime, bool isRACH = false)
		: mHighSideFEC(wHighSideFEC), mStartTime(wStartTime), mLastDetectedTime(Time(0, 0)),
		  mFirstDetectedTime(Time(0, 0)), mRACH(isRACH), mLastTOA(0.0){};
