...
static SlotFormat Table11(
	int format, int kbps, int sf, int bitsPerSlot, int data1, int data2, int tpc, int tfci, int pilot)
{
	SlotFormat result;
...
SlotFormat SlotInfoDownlinkDch[17] = {
	...
	Table11(0, 15, 512, 10, 0, 4, 2, 0, 4), Table11(1, 15, 512, 10, 0, 2, 2, 2, 4),
	Table11(2, 30, 256, 20, 2, 14, 2, 0, 2), Table11(3, 30, 256, 20, 2, 12, 2, 2, 2),
	Table11(4, 30, 256, 20, 2, 12, 2, 0, 4), Table11(5, 30, 256, 20, 2, 10, 2, 2, 4),
	Table11(6, 30, 256, 20, 2, 8, 2, 0, 8), Table11(7, 30, 256, 20, 2, 6, 2, 2, 8),
	Table11(8, 60, 128, 40, 6, 28, 2, 0, 4), Table11(9, 60, 128, 40, 6, 26, 2, 2, 4),
	Table11(10, 60, 128, 40, 6, 24, 2, 0, 8), Table11(11, 60, 128, 40, 6, 22, 2, 2, 8),
	Table11(12, 120, 64, 80, 12, 48, 4, 8, 8), Table11(13, 240, 32, 160, 28, 112, 4, 8, 8),
	Table11(14, 480, 16, 320, 56, 232, 8, 8, 16), Table11(15, 960, 8, 640, 120, 488, 8, 8, 16),
	Table11(16, 1920, 4, 1280, 248, 1000, 8, 8, 16)};

SlotFormat SlotInfoDownlinkDchREL99[17] = {
	// format       kbps    sf      b/slot  data1   data2   tpc     tfci    pilot
	Table11(0, 15, 512, 10, 2, 2, 2, 0, 4), Table11(1, 15, 512, 10, 0, 2, 2, 2, 4),
	Table11(2, 30, 256, 20, 2, 14, 2, 0, 2), Table11(3, 30, 256, 20, 0, 14, 2, 2, 2),
	Table11(4, 30, 256, 20, 2, 12, 2, 0, 4), Table11(5, 30, 256, 20, 0, 12, 2, 2, 4),
	Table11(6, 30, 256, 20, 2, 8, 2, 0, 8), Table11(7, 30, 256, 20, 0, 8, 2, 2, 8),
	Table11(8, 60, 128, 40, 6, 28, 2, 0, 4), Table11(9, 60, 128, 40, 4, 28, 2, 2, 4),
	Table11(10, 60, 128, 40, 6, 24, 2, 0, 8), Table11(11, 60, 128, 40, 4, 24, 2, 2, 8),
	Table11(12, 120, 64, 80, 4, 56, 4, 8, 8), Table11(13, 240, 32, 160, 20, 120, 4, 8, 8),
	Table11(14, 480, 16, 320, 48, 240, 8, 8, 16), Table11(15, 960, 8, 640, 112, 496, 8, 8, 16),
...
static SlotFormat Tablec(int format, int kbps, int sf, int bitsPerSlot, int pilot, int tpc, int tfci, int fbi)
{
	...
	result.mNTfci = tfci;
	result.mNTpc = tpc;
	assert(fbi + result.mNData1 + result.mNData2 + result.mNPilot + result.mNTfci + result.mNTpc ==
		result.mBitsPerSlot);
	return result;
}
...
SlotFormat SlotInfoUplinkDpdch[7] = {
	...
	Tabled(0, 15, 256, 10), Tabled(1, 30, 128, 20), Tabled(2, 60, 64, 40), Tabled(3, 120, 32, 80),
	Tabled(4, 240, 16, 160), Tabled(5, 480, 8, 320), Tabled(6, 960, 4, 640)};

// 3GPP 25.211 Table 2: uplink DPCCH [control] fields
SlotFormat SlotInfoUplinkDpcch[5] = {
	// format	kbs		sf		b/slot	pilot	tpc	tfci fbi
	Tablec(0, 15, 256, 10, 6, 2, 2, 0), Tablec(1, 15, 256, 10, 8, 2, 0, 0), Tablec(2, 15, 256, 10, 5, 2, 2, 1),
	Tablec(3, 15, 256, 10, 7, 2, 0, 1), Tablec(4, 15, 256, 10, 6, 4, 0, 0)};
...
SlotFormat SlotInfoSccpch[18] = {
	...
	Table18(0, 30, 256, 20, 20, 0, 0), Table18(1, 30, 256, 20, 12, 8, 0), Table18(2, 30, 256, 20, 18, 0, 2),
	Table18(3, 30, 256, 20, 10, 8, 2), Table18(4, 60, 128, 40, 40, 0, 0), Table18(5, 60, 128, 40, 32, 8, 0),
	Table18(6, 60, 128, 40, 38, 0, 2), Table18(7, 60, 128, 40, 30, 8, 2), Table18(8, 120, 64, 80, 72, 0, 8),
	Table18(9, 120, 64, 80, 64, 8, 8), Table18(10, 240, 32, 160, 152, 0, 8), Table18(11, 240, 32, 160, 144, 8, 8),
	Table18(12, 480, 16, 320, 312, 0, 8), Table18(13, 480, 16, 320, 296, 16, 8),
	Table18(14, 960, 8, 640, 632, 0, 8), Table18(15, 960, 8, 640, 616, 16, 8),
	Table18(16, 1920, 4, 1280, 1272, 0, 8), Table18(17, 1920, 4, 1280, 1256, 16, 8)};
...
PhCh::PhCh(PhChType chType,
	...
	ARFCNManager *wRadio)
	: mPhChType(chType), mDlSF(wDlSF), mDlSFLog2((int)log2(wDlSF)), mUlSF(wUlSF), mSpCode(wSpCode),
	  mSrCode(wSrCode), mRadio(wRadio), mUlPuncturingLimit(gConfig.getNum("UMTS.Uplink.Puncturing.Limit")),
	  mAllocated(0), mDlSlot(0), mUlDPCCH(0)
{
	...
	case PCCPCHType:			      // used for BCH
		...
		// gChannelTree.chReserve(wDlSF, wSpCode);	// Redundant; this ch is fixed and we reserve it
		// elsewhere too.
		break;
	...
	if (chType == DPDCHType)
		mUlDPCCH = &SlotInfoUplinkDpcch[0];
...
unsigned PhCh::getUlRadioFrameSize() { return gFrameLen / mUlSF; }
...
void PhCh::toAsnUL_DPCH_Info(ASN::UL_DPCH_Info *iep)
{
	...
	iep->ul_DPCH_PowerControlInfo->choice.fdd.powerControlAlgorithm.present =
		ASN::PowerControlAlgorithm_PR_algorithm2;
	...
	// Gotta love this...
	struct ASN::UL_DPCH_Info::UL_DPCH_Info__modeSpecificInfo::UL_DPCH_Info__modeSpecificInfo_u::
...
{
	if (this->mPhChType != DPDCHType) {
		return NULL;
	} // This IE applies only to DPCH.
...
ASN::DL_DPCH_InfoCommon *PhCh::toAsnDL_DPCH_InfoCommon()
{
	...
	ASN::DL_DPCH_InfoCommon::DL_DPCH_InfoCommon__modeSpecificInfo::DL_DPCH_InfoCommon__modeSpecificInfo_u::
	...
	switch (this->mDlSF) {
	case 4:
		fdd->spreadingFactorAndPilot.present = ASN::SF512_AndPilot_PR_sfd4;
		break;
	case 8:
		fdd->spreadingFactorAndPilot.present = ASN::SF512_AndPilot_PR_sfd8;
		break;
	case 16:
		fdd->spreadingFactorAndPilot.present = ASN::SF512_AndPilot_PR_sfd16;
		break;
	case 32:
		fdd->spreadingFactorAndPilot.present = ASN::SF512_AndPilot_PR_sfd32;
		break;
	case 64:
		fdd->spreadingFactorAndPilot.present = ASN::SF512_AndPilot_PR_sfd64;
		break;
	case 128:
...
ASN::DL_DPCH_InfoPerRL *PhCh::toAsnDL_DPCH_InfoPerRL()
{
	...
	int asncuval =
		cpichUsage ? ASN::PCPICH_UsageForChannelEst_mayBeUsed : ASN::PCPICH_UsageForChannelEst_shallNotBeUsed;
...
{
	if (checkOnlyReserved) {
		return !mReserved;
	}
	return !mReserved && !mAlsoReserved && mDch && !mDch->phChAllocated();
}

bool ChannelTreeElt::active(void) { return mReserved || !mDch || mDch->phChAllocated(); }

void ChannelTree::chConflict(Tier t1, unsigned ch1, Tier t2, unsigned ch2)
{
	LOG(ALERT) << "Attempt to reserve channel:" << LOGVAR2("sf", tier2sf(t1)) << LOGVAR2("chcode", ch1)
		   << " which conflicts with reserved channel:" LOGVAR2("sf", tier2sf(t2)) << LOGVAR2("chcode", ch2);
	// assert(0);
}
...
void ChannelTree::chReserve(unsigned sf, unsigned chcode)
{
	...
	Tier t = sf2tier(sf);
	Tier badtier;
	unsigned badcode; // To hold a conflicing reservation.
...
ChannelTree::Tier ChannelTree::bw2tier(unsigned ops, bool guaranteed)
{
	...
	if (guaranteed) {
		if (ops >= 120000) {
			return 0;
		} // SF=4, actual == 240K, less overhead
		if (ops >= 60000) {
			return 1;
		} // SF=8, actual == 120K
		if (ops >= 30000) {
...
bool ChannelTree::isTierFreeUpward(
	Tier tier, unsigned startcode, bool checkOnlyReserved, Tier *badtier, unsigned *badcode)
{
...
bool ChannelTree::isTierFreeDownward(
	Tier tier, unsigned startcode, unsigned width, bool checkOnlyReserved, Tier *badtier, unsigned *badcode)
{
	if (tier >= sNumTiers) {
		return true;
	}
	...
	for (unsigned i = 0; i < width; i++, code++) {
		// redundant with available(): if (mTree[tier][code].mReserved) {return false;}
		...
		if (!isTierFreeDownward(tier + 1, 2 * startcode, 2 * width, checkOnlyReserved, badtier, badcode)) {
			return false;
		} // something underneath is in use.
	}
...
DCHFEC *ChannelTree::chChooseByTier(Tier tier)
{
	...
	for (unsigned chcode = 0; chcode < sf; chcode++) {
		// Harvind (3-11-13) upward search should only check reserved codes, don't care if upward codes are
		// "also reserved"
...
DCHFEC *ChannelTree::chChooseBySF(unsigned sf) { return chChooseByTier(sf2tier(sf)); }
...
void ChannelTree::chTestAlloc(int sf, int cnt, std::ostream &os)
{
	int i;
	for (i = 0; i < cnt; i++) {
		if (!gChannelTree.chChooseBySF(sf))
			break;
	}
