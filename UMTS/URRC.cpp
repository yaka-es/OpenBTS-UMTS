...
void RBInfo::defaultConfig3Rb(unsigned rbn)
{
	...
	case 5:
	case 6:
	case 7: // These are the AMR voice RBs.
		ul_RLC_Mode(URlcModeTm);
		ul_segmentationIndication(false);
	...
	case 1:
	case 2:
	case 3: // SRB1,2,3 multiplexed on TrCh 4.
		setTransportChannelIdentity(4);
		break;
	case 5: // AMR voice channels on RB5,6,7 mapped to trch1,2,3
		setTransportChannelIdentity(1);
		break;
	case 6:
		setTransportChannelIdentity(2);
		break;
	case 7:
		setTransportChannelIdentity(3);
		break;
	default:
		assert(0);
	}
	...
	case 2:
	case 3:
	case 4: // All SRBs on TrCh 4, which is multiplexed.
...
void UEInfo::ueRegisterActivity() { mActivityTime.now(); }
...
void Rrc::purgeUEs()
{
	...
	RN_FOR_ITR(UEList_t, mUEList, itr)
	{
		...
		***/
		...
		case stCELL_DCH:
		case stCELL_PCH:
			if (elapsed > tInactivity && last->mTransactionType != ttRrcConnectionRelease) {
...
UEInfo *Rrc::findUe(bool ueidtypeCRNTI, unsigned uehandle)
{
	...
	{
		if (ueidtypeCRNTI) {
			if (uehandle == uep->mCRNTI) {
				return uep;
			}
		} else {
			if (uehandle == uep->mURNTI) {
				return uep;
			}
		}
	}
...
UEInfo *Rrc::findUeByAsnId(AsnUeId *asnId)
{
	...
	LOG(ALERT) << "no match" << LOGHEX2("ptmsi", (uint32_t)asnId->mPtmsi)
		   << LOGVAR2("raimatch", asnId->RaiMatches()) << LOGHEX2("findHandlebyPTmsi-urnti", urnti);
	return NULL;
}
...
{
	return format("UeTransaction(type=%d=%s newState=%s rabMask=%d transId=%d)", mTransactionType,
		TransType2Name(mTransactionType), UEState2Name(mNewState), mRabMask, mTransactionId);
}
...
{
	if (mNextTransactionId == 0)
		return NULL; // No transactions have occurred yet.
...
{
	if (transId >= 4) {
		PATLOG(1, format("%s: invalid transaction id, out of range, transId:%d", help, transId));
		return NULL;
	}
...
{
	if (transId >= 4) {
		PATLOG(1, format("%s: invalid transaction id, out of range, type:%d transId:%d", help, ttype, transId));
		return NULL;
	}
	UeTransaction *tr = &mTransactions[transId];
	if (tr->mTransactionType != ttype) {
		PATLOG(1, format("%s: invalid transaction, expected type:%d got type:%d id:%d", help, ttype,
				  tr->mTransactionType, transId));
		return NULL;
	}
	return tr;
}
...
DCCHLogicalChannel *UEInfo::allocateLogicalChannel() { return new DCCHLogicalChannel(this); }
...
void UEInfo::ueRecvRadioBearerReleaseResponse(unsigned transId, bool success, const char *msgname)
{
	...
	for (RbId rbid = 5; rbid < gsMaxRB; rbid++) {
		if (tr->mRabMask & (1 << rbid)) {
			mConfiguredRabs[rbid].mStatus =
				success ? SGSN::RabStatus::RabIdle : SGSN::RabStatus::RabFailure;
...
void UEInfo::ueRecvRadioBearerSetupResponse(unsigned transId, bool success, const char *msgname)
{
	...
	for (RbId rbid = 5; rbid < gsMaxRB; rbid++) {
		if (tr->mRabMask & (1 << rbid)) {
			mConfiguredRabs[rbid].mStatus =
				success ? SGSN::RabStatus::RabAllocated : SGSN::RabStatus::RabFailure;
...
TrChConfig *UEInfo::ueGetTrChConfig() { return &ueGetConfig()->mTrCh; }
...
{
	RN_UE_FOR_ALL_RLC_DOWN(this, rbid, rlcp) { rlcp->rlcPullLowSide(amt); }
}
...
void UEInfo::ueWriteLowSide(RbId rbid, const BitVector &pdu, UEState state)
{
	...
	if (mUeState == stIdleMode) {
		...
		// FIXME: This may have missed the uplink ConnectionSetupComplete message, in which case what should we
		// do?
		...
		if (trans && trans->mTransactionType == UeTransaction::ttRrcConnectionSetup) {
			...
			LOG(ERR) << "Received DCCH message for UE in idle mode for trans. type: "
				 << trans->mTransactionType << " " << this;
		}
	}
	URlcRecv *rlc = NULL;
	if (rbid <= mUeMaxRlc) {
...
void UEInfo::ueDisconnectRlc(UEState state)
{
	...
	if (cnt)
		LOG(INFO) << format("ueDisconnectRlc: new state=%s disconnected %d deleted %d %s", UEState2Name(state),
			cnt, deleted, this->ueid().c_str());
}
...
// the state change.
void UEInfo::ueConnectRlc(RrcMasterChConfig *config, // This is the config we will use in nextState.
	...
	for (unsigned i = 1; i < config->mNumRB; i++) { // Skip SRB0
		...
		if (rb->valid()) {
			...
			case stCELL_FACH:
				...
				if (!isNewState && prev) {
					continue;
				} // Leave previously configured RBs alone.
				other = mRlcsCDch[rbid];
				if (prev && prev != other) {
					delete prev;
				}
				if (other && other->mDown->mRlcMode == rb->getDlRlcMode() &&
			...
			LOG(INFO) << format("ueConnectRlc:curstate=%s next=%s %s i=%d rbid=%d up=%s down=%s %s",
				UEState2Name(ueGetState()), UEState2Name(nextState), action, i, rbid,
				URlcMode2Name(rb->getUlRlcMode()), URlcMode2Name(rb->getDlRlcMode()),
				this->ueid().c_str());
		}
	}
...
static SGSN::RabStatus rrcAllocateRabForPdp(uint32_t urnti, int rbid, ByteVector &qosb)
{
	...
	if (dch) {
		...
		if (uep->ueGetState() != stCELL_DCH) {
			...
			LOG(ERR) << "UE has DCH but is not in CELL_DCH state"
				 << " ue:" << uep;
		}
	} else {
...
unsigned R3EncodedSize(unsigned Ki)
{
	assert(0);
	return 3 * Ki + 24;
} // rate 1/3
unsigned R3DecodedSize(unsigned Yi)
{
	assert(0);
	return (Yi - 24) / 3;
} // rate 1/3
...
unsigned RrcDefs::TurboDecodedSize(unsigned Yi) // rate 1/2: base formula is: Ki = (Yi-16)/2;
{
	...
	unsigned Yi2;
	while ((Yi2 = TurboEncodedSize(Ki)) > Yi) {
		Ki--;
	} // Search for best.
...
unsigned RrcDefs::R2EncodedSize(unsigned Xi, unsigned *codeBkSz, unsigned *fillBits)
{
	...
	int Ki =
		(Ci == 0) ? 0 : (Xi + Ci - 1) / Ci; // number of bits per block, including filler bits. Ki = ceil(Xi/Ci)
	// If Ci=3, Xi=3,Ki=1 Xi=4,Ki=2
	if (fillBits)
		*fillBits = Ci * Ki - Xi; // number of filler bits, unused here.
	if (codeBkSz)
		*codeBkSz = Ki;
...
unsigned RrcDefs::TurboEncodedSize(unsigned Xi, unsigned *codeBkSz, unsigned *fillBits)
{
	...
	if (fillBits)
		*fillBits = Ci * Ki - Xi; // number of filler bits, unused here.
	if (codeBkSz)
		*codeBkSz = Ki;    // Size of each coded block.
...
namespace SGSN {
MSUEAdapter *SgsnAdapter::findMs(uint32_t urnti) { return UMTS::gRrc.findUeByUrnti(urnti); }

bool SgsnAdapter::isUmts() { return true; }
