...
private:
	...
	static const radioData_t mPSCHAmplitude =
		2; // usually 3dB below CPICH, but can be signifcantly lower (PSCH not scrambled)
	static const radioData_t mSSCHAmplitude = 5; // usually 3dB below CPICH (keep in mind SSCH not scrambled)
	static const radioData_t mCCPCHAmplitude =
		2; // e.g. typically CPCCH is 5 dB below CPICH, AICH level is set in SIB5, etc.
	...
	float estimateChannel(signalVector *wBurst, signalVector *matchedFilter, unsigned maxTOA, unsigned startTOA,
	...
	void scramble(radioData_t *wBurstI, radioData_t *wBurstQ, int len, int8_t *codeI, int8_t *codeQ, int codeLen,
		radioData_t **rBurstI, radioData_t **rBurstQ);

	/* Used to scramble pre-computed RACH waveforms */
	void scrambleRACH(radioData_t *wBurstI, int len, int8_t *codeI, int codeLen, radioData_t **rBurstI);

	/* Descramble a receive burst...essentially a series of sign changes on array of floats?  Nope, they are complex
	 * multiplies.*/
	...
	signalVector *despread(signalVector &wBurst, const int8_t *code, int codeLength, bool useQ);

	/* Spread a scrambled burst...essentially an Kronecker product */
	void spread(BitVector &wBurst, int8_t *code, int codeLen, radioData_t *accI, radioData_t *accQ, int accLen,
		radioData_t gain = 1);
...
public:
	...
	bool decodeDCH(signalVector &wBurst, UMTS::Time wTime, int uplinkScramblingCodeIndex, int numPilots,
		signalVector &descrambledBurst, signalVector &rawBurst, float &guessTOA, float &bestTOA,
		complex &bestChannel, float &bestSNR, float *TFCI, float *TPC);

	bool decodeDPDCHFrame(DPDCH &frame, int uplinkScramblingCodeIndex, int uplinkSpreadingFactorLog2,
