...
class BeaconConfig {
	...
	void encodeSIBPhase1(unsigned sibIndex, // Index into mSibInfo.
		const char *sibId, // The RRC spec SIB type number used only for user readable messages, may be double
				   // in future.
		struct ASN::asn_TYPE_descriptor_s *type_descriptor, void *struct_ptr); /* Structure to be encoded */
...
public:
	...
	TransportBlock *getSITB(unsigned sfn)
	{ // Get System Information Transport Block.
		assert(!(sfn & 1));
...
BeaconConfig::BeaconConfig() : mMIBValueTag(0), mNumSibTypes(0)
{
...
#define SIB_TAGS(n) ASN::SIB_Type_systemInformationBlockType##n, ASN::SIBSb_TypeAndTag_PR_sysInfoType##n
...
static int getConfigSccpchSF() { return gConfig.getNum("UMTS.SCCPCH.SF"); }

static int getConfigPrachSF() { return gConfig.getNum("UMTS.PRACH.SF"); }
...
{
	if (gConfig.defines("UMTS.Debug")) {
		rrcDebugLevel = gConfig.getNum("UMTS.Debug.RRC");
	}
...
void BeaconConfig::encodeSIBPhase1(unsigned sibIndex, const char *sibId, struct asn_TYPE_descriptor_s *type_descriptor,
	void *struct_ptr) /* Structure to be encoded */
{
	bool rn_asn_debug_beacon = gConfig.getNum("UMTS.Debug.ASN.Beacon");
	if (rn_asn_debug_beacon) {
		printf("=== Phase1 Encoding SIB %s\n", sibId);
	}
	...
	asn_enc_rval_t rval =
		uper_encode_to_buffer(type_descriptor, struct_ptr, (void *)perBuf->begin(), perBuf->size());
...
void BeaconConfig::encodeSI(const char *sibId, SystemInformation_BCH &si, TransportBlock *trb, unsigned sfn)
{
	...
	// (10-5-2012 pat) The encoder output appears to have interjected randomness, so lets try pre-zeroing out the
	// buffer: Update: that did the trick.
	...
	if (sizeBits > (int)trb->size()) {
		LOG(ERR) << "SIB " << sibId << " size " << sizeBits << " does not fit BCH transfer block "
			 << trb->size();
	}
...
void BeaconConfig::encodeSIBPhase2(SibInfo_t *plan, unsigned sfn)
{
...
#if PAT_TEST
			if (numBits < 222) {
				in->setField(numBits, 0, 222 - numBits);
			} // (pat) zero pad the tail.
#endif
...
static SCCPCH_SystemInformation *generateSCCPCH(unsigned fachChCode, bool addPICH, unsigned pichChCode)
{
	...
	asn_long2INTEGER(&(sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.dummy1),
		PCPICH_UsageForChannelEst_mayBeUsed);
	...
	switch (scc_sf) {
	case 256:
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.present =
			SF256_AndCodeNumber_PR_sf256;
		break;
	case 128:
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.present =
			SF256_AndCodeNumber_PR_sf128; // 256;
		break;
	case 64:
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.present =
			SF256_AndCodeNumber_PR_sf64; // 256;
		break;
	case 32:
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.present =
			SF256_AndCodeNumber_PR_sf32; // 256;
		break;
	case 16:
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.present =
			SF256_AndCodeNumber_PR_sf16; // 256;
		break;
	case 8:
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.present =
			SF256_AndCodeNumber_PR_sf8; // 256;
		break;
	case 4:
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.present =
			SF256_AndCodeNumber_PR_sf4; // 256;
		break;
	default:
		break;
	}
	...
	asn_long2INTEGER(&(sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.positionFixedOrFlexible),
		PositionFixedOrFlexible_fixed);
...
#if 1 // PAT_TEST
      // (pat) It is 1, not 3.
	fachPchInfo->transportChannelIdentity =
		gRrcDcchConfig->getDlTrChInfo(0)->mTransportChannelIdentity; // Range is 1..32
	...
	// FIXME: Adding second FACH to SIB5, though it doesn't really exist.  Phone assumes first FACH is PCH and won't
	// go into the RACH procedure
#if 0 // unused code
...
void BeaconConfig::regenerate()
{
	...
	setASN1SeqOfDigits((void *)&mMIB.plmn_Type.choice.gsm_MAP.plmn_Identity.mcc.list, MCC.c_str());
	const string MNC = gConfig.getStr("UMTS.Identity.MNC");
	setASN1SeqOfDigits((void *)&mMIB.plmn_Type.choice.gsm_MAP.plmn_Identity.mnc.list, MNC.c_str());
	...
	for (unsigned i = 1; i < mNumSibTypes; i++) { // Skip 0, which is for the MIB
		...
		switch (plan->mSibRep) {
		case 8:
			SIBSb->scheduling.scheduling.sib_Pos.present =
	...
	*LAC = htons(gConfig.getNum("UMTS.Identity.LAC"));
	...
	if (1) {
		...
		// DRX_CycleLengthCoeff: "A coefficient in the formula to count the paging occasions to be used by a
		// specific UE (specified
		//	in  3GPP TS 25.304: "UE Procedures in Idle Mode and Procedures for Cell Reselection in Connected
		// Mode".)
		dsi2->cn_DRX_CycleLengthCoeff =
			gConfig.getNum("UMTS.CN-DSI.CycleLengthCoeff"); // FIXME -- What does this mean?!
	...
	connTimers->t_301 = RN_CALLOC(ASN::T_301_t);
	asn_long2INTEGER(connTimers->t_301, ASN::T_301_ms2000);
	connTimers->n_301 = RN_CALLOC(ASN::N_301_t);
	*(connTimers->n_301) = 2;
	connTimers->t_302 = RN_CALLOC(ASN::T_302_t);
	asn_long2INTEGER(connTimers->t_302, ASN::T_302_ms4000);
	connTimers->n_302 = RN_CALLOC(ASN::N_302_t);
	*(connTimers->n_302) = 3;
	connTimers->t_304 = RN_CALLOC(ASN::T_304_t);
	asn_long2INTEGER(connTimers->t_304, ASN::T_304_ms2000);
	connTimers->n_304 = RN_CALLOC(ASN::N_304_t);
	*(connTimers->n_304) = 2;
	connTimers->t_305 = RN_CALLOC(ASN::T_305_t);
	asn_long2INTEGER(connTimers->t_305, ASN::T_305_m30);
	connTimers->t_307 = RN_CALLOC(ASN::T_307_t);
	asn_long2INTEGER(connTimers->t_307, ASN::T_307_s30);
	connTimers->t_308 = RN_CALLOC(ASN::T_308_t);
	asn_long2INTEGER(connTimers->t_308, ASN::T_308_ms320);
	connTimers->t_309 = RN_CALLOC(ASN::T_309_t);
	*(connTimers->t_309) = 5;
	connTimers->t_310 = RN_CALLOC(ASN::T_310_t);
	asn_long2INTEGER(connTimers->t_310, ASN::T_310_ms160);
	connTimers->n_310 = RN_CALLOC(ASN::N_310_t);
	*(connTimers->n_310) = 4;
	connTimers->t_311 = RN_CALLOC(ASN::T_311_t);
	asn_long2INTEGER(connTimers->t_311, ASN::T_311_ms2000);
	connTimers->t_312 = RN_CALLOC(ASN::T_312_t);
	*(connTimers->t_312) = 1;
	connTimers->n_312 = RN_CALLOC(ASN::N_312_t);
	asn_long2INTEGER(connTimers->n_312, ASN::N_312_s1);
	connTimers->t_313 = RN_CALLOC(ASN::T_313_t);
	*(connTimers->t_313) = 3;
	connTimers->n_313 = RN_CALLOC(ASN::N_313_t);
	asn_long2INTEGER(connTimers->n_313, ASN::N_313_s20);
	connTimers->t_314 = RN_CALLOC(ASN::T_314_t);
	asn_long2INTEGER(connTimers->t_314, ASN::T_314_s0);
	connTimers->t_315 = RN_CALLOC(ASN::T_315_t);
	asn_long2INTEGER(connTimers->t_315, ASN::T_315_s0);
	connTimers->n_315 = RN_CALLOC(ASN::N_315_t);
	asn_long2INTEGER(connTimers->n_315, ASN::N_315_s1);
	connTimers->t_316 = RN_CALLOC(ASN::T_316_t);
	asn_long2INTEGER(connTimers->t_316, ASN::T_316_s30);
	connTimers->t_317 = RN_CALLOC(ASN::T_317_t);
	asn_long2INTEGER(connTimers->t_317, ASN::T_317_infinity1);
	...
	*uraval = htons(gConfig.getNum("UMTS.Identity.URAI"));
	...
	// UC-Id or C-Id is used to identify a cell in UTRAN Iub and Iur interfaces or Iur-g interface: UC-Id = RNC-Id +
	// C-Id. (pat) This last does not apply to us because we dont use the Iub/Iur interfaces.
	...
	*cellID = htonl(gConfig.getNum("UMTS.Identity.CI"));
	...
	mSIB3.cellSelectReselectInfo.modeSpecificInfo.choice.fdd.q_QualMin =
		-24; // gConfig.getNum("UMTS.CellSelect.QualMin", -12);
	mSIB3.cellSelectReselectInfo.modeSpecificInfo.choice.fdd.q_RxlevMin =
		-58; // gConfig.getNum("UMTS.CellSelect.Q-RxlevMin",-20);
	...
	{ // start of 10.3.6.55 prach_SystemInformationList, with one entry
		...
		*PRACHSigs = htons(0x0001 << gConfig.getNum("UMTS.PRACH.Signature"));
		setAsnBIT_STRING(&prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.availableSignatures,
			(uint8_t *)PRACHSigs, 16);
		//   spreading factor
		unsigned rachSF = gConfig.getNum("UMTS.PRACH.SF");
		switch (rachSF) {
		case 256:
			asn_long2INTEGER(
				&(prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.availableSF), SF_PRACH_sfpr256);
			break;
		case 128:
			asn_long2INTEGER(
				&(prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.availableSF), SF_PRACH_sfpr128);
			break;
		case 64:
			asn_long2INTEGER(
				&(prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.availableSF), SF_PRACH_sfpr64);
			break;
		case 32:
			asn_long2INTEGER(
				&(prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.availableSF), SF_PRACH_sfpr32);
			break;
		default:
			break;
		}
		//   scrambling code
		prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.preambleScramblingCodeWordNumber =
			gConfig.getNum("UMTS.PRACH.ScramblingCode");
		...
		asn_long2INTEGER(
			&(prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.puncturingLimit), PuncturingLimit_pl1);
		...
		*PRACHSubChan = htons(0x0010 << gConfig.getNum("UMTS.PRACH.Subchannel"));
		...
		setAsnBIT_STRING(&prach_SI->prach_RACH_Info.modeSpecificInfo.choice.fdd.availableSubChannelNumbers,
			(uint8_t *)PRACHSubChan, 12);
...
#if PAT_TEST
		// (pat) It is 1, not 3.
		prach_SI->transportChannelIdentity =
			gRrcDcchConfig->getUlTrChInfo(0)->mTransportChannelIdentity; // Range is 1..32
		...
		gRrcCcchConfig->getUlTfs()->toAsnTfs(
			(prach_SI->rach_TransportFormatSet = RN_CALLOC(ASN::TransportFormatSet)));
		gRrcCcchConfig->getUlTfcs()->toAsnTfcs((prach_SI->rach_TFCS = RN_CALLOC(ASN::TFCS)), TrChRACHType);

		// 10.3.6.53 prach_Partitioning - optional (pat) It defaults to all ASC are available, which is what we
		// want. (pat) If you take out prach_partitioning, the Samsung does not RACH, or if it does, something
		// is wrong.
		...
		*prach_SI->modeSpecificInfo.choice.fdd.constantValue = -10;
		...
		prach_SI->modeSpecificInfo.choice.fdd.rach_TransmissionParameters =
			RN_CALLOC(ASN::RACH_TransmissionParameters);
		...
		if (cAICHRACHOffset == 5) {
			asn_long2INTEGER(&(prach_SI->modeSpecificInfo.choice.fdd.aich_Info->aich_TransmissionTiming),
				AICH_TransmissionTiming_e1);
		} else {
	...
	{ // start of sCCPCH_SystemInformationList
	  ...
	  // (pat) See 10.3.6.72: Secondary CCPCH Info IE.
		...
		// We are assigning to choice.sf256 but it is the same variable location for all SFs.
		sCCPCH_SI->secondaryCCPCH_Info.modeSpecificInfo.choice.fdd.sf_AndCodeNumber.choice.sf256 =
			fachChCode + 1;
		...
		sCCPCH_SI->fach_PCH_InformationList = RN_CALLOC(ASN::FACH_PCH_InformationList);
		ASN_SEQUENCE_ADD(&sCCPCH_SI->fach_PCH_InformationList->list, fachPchInfo);
		// FIXME: Adding second FACH to SIB5, though it doesn't really exist.  Phone assumes first FACH is PCH
		// and won't go into the RACH procedure
#if 0 // unused code
	/*FACH_PCH_Information *fachPchInfo2 = RN_CALLOC(ASN::FACH_PCH_Information);
	memcpy(fachPchInfo2,fachPchInfo,sizeof(ASN::FACH_PCH_Information));
	fachPchInfo2->transportChannelIdentity = 2;
		...
		// These are different channels.  They all use the transport channel id in the TFS.  Same TFS is used
		// for all.
	...
	mSIB11.measurementControlSysInfo.use_of_HCS.choice.hcs_not_used.cellSelectQualityMeasure.present =
...
void BeaconConfig::encodePhase2(unsigned sfn)
{
	...
	if (sfn == sPrevBeaconStart) {
		return;
	} // Already encoded this beacon cycle.
...
ChanType *getChan(vector<ChanType *> &chanList)
{
...
template <class ChanType>
unsigned countActive(const vector<ChanType *> &chanList)
{
	unsigned active = 0;
	const unsigned sz = chanList.size();
	// Start the search from a random point in the list.
...
unsigned UMTSConfig::DTCHActive() const { return countActive(mDTCHPool); }
