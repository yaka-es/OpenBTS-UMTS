...
extern bool uperEncodeToBV(
	ASN::asn_TYPE_descriptor_t *td, void *sptr, ByteVector &result, const std::string descr = "");
...
class UEInfo;
void asnLogMsg(unsigned rbid, ASN::asn_TYPE_descriptor_t *asnp, const void *struct_ptr, const char *comment = "",
	UEInfo *uep = NULL, uint32_t urnti = 0);
...
public:
	...
	void cvtAsn(ASN::ENUMERATED_t *pasnval, long actual) { asn_long2INTEGER(pasnval, findEnum(actual)); }
	void cvtAsn(ASN::ENUMERATED_t &asnval, long actual) { cvtAsn(&asnval, actual); }

	ASN::ENUMERATED_t toAsn(long actual) { return toAsnEnumerated(findEnum(actual)); }
...
struct AsnBitString2BV : public ByteVector {
	...
	{
		if (ptr->bits_unused)
			setSizeBits(ptr->size * 8 - ptr->bits_unused);
	}
