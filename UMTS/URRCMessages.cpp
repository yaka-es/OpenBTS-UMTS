...
static const char *asnUlDcchMsg2Name(ASN::UL_DCCH_MessageType_PR mtype)
{
	...
	default:
		return "UL_DCCH_unrecognized_message_type";
	}
}
...
static string getRBFailureCause(ASN::FailureCauseWithProtErr_t *failureCause)
{
	...
	case ASN::FailureCauseWithProtErr_PR_configurationUnsupported:
		msg = "configurationUnsupported";
		break;
	case ASN::FailureCauseWithProtErr_PR_physicalChannelFailure:
		msg = "physicalChannelASN::Failure";
		break;
	...
	case ASN::FailureCauseWithProtErr_PR_compressedModeRuntimeError:
		msg = "compressedModeRuntimeError";
		break;
...
ASN::RRC_StateIndicator UEState2Asn(UEState state)
{
	...
	case stCELL_FACH:
		return ASN::RRC_StateIndicator_cell_FACH;
	case stCELL_DCH:
		return ASN::RRC_StateIndicator_cell_DCH;
	case stCELL_PCH:
		return ASN::RRC_StateIndicator_cell_PCH;
	case stURA_PCH:
		return ASN::RRC_StateIndicator_ura_PCH;
	default:
		assert(0);
	}
}
...
static bool encodeDcchMsg(UEInfo *uep, RbId rbid, ASN::DL_DCCH_Message_t *msg, ByteVector &result, string descr)
{
	if (uep->integrity.isStarted()) {
		...
		if (!uperEncodeToBV(&ASN::asn_DEF_DL_DCCH_Message, msg, result, descr)) {
			return false;
		}
...
static void toAsnSRB_InformationSetupList(RrcMasterChConfig *masterConfig, void *srblist)
{
	...
	for (unsigned rbid = SRB1; rbid <= SRB3; rbid++) {
		...
		*srbie->rb_Identity = rbid;
		...
		defaultRbMappingInfoToAsn(masterConfig->getUlTrChInfo(tcid), masterConfig->getDlTrChInfo(tcid), rbid,
			&srbie->rb_MappingInfo);
...
static ASN::RB_InformationAffectedList *toAsnRB_InformationAffectedList(RrcMasterChConfig *masterConfig)
{
...
void toAsnRAB_Info(RBInfo *rb, ASN::RAB_Info_t *rabInfoIE)
{
	...
	rabInfoIE->cn_DomainIdentity =
		toAsnEnumerated(csdomain ? ASN::CN_DomainIdentity_cs_domain : ASN::CN_DomainIdentity_ps_domain);
	...
	rabInfoIE->re_EstablishmentTimer =
		toAsnEnumerated(csdomain ? ASN::Re_EstablishmentTimer_useT314 : ASN::Re_EstablishmentTimer_useT315);
...
static ASN::RB_InformationSetup *toAsnRB_InformationSetup(RrcMasterChConfig *masterConfig, RBInfo *rb)
{
	...
	defaultRbMappingInfoToAsn(
		masterConfig->getUlTrChInfo(tcid), masterConfig->getDlTrChInfo(tcid), rbid, &rbie->rb_MappingInfo);
	return rbie;
}
...
static ASN::RAB_InformationSetup *toAsnRAB_InformationSetup(RrcMasterChConfig *masterConfig, int rbid,
...
static void toAsnRAB_InformationSetupList(RrcMasterChConfig *masterConfig, ASN::RAB_InformationSetupList *rbSetupListIE)
{
	...
	for (unsigned rbid = 5; rbid < masterConfig->mNumRB;) {
		...
		int numRBperRAB;
		ASN::RAB_InformationSetup *rabSetupIE = toAsnRAB_InformationSetup(masterConfig, rbid, &numRBperRAB);
...
static ASN::DL_CommonTransChInfo *toAsnDL_CommonTransChInfo(RrcMasterChConfig *masterConfig)
{
	...
	typedef ASN::DL_CommonTransChInfo::DL_CommonTransChInfo__modeSpecificInfo::
		DL_CommonTransChInfo__modeSpecificInfo_u::DL_CommonTransChInfo__modeSpecificInfo__fdd::
...
ASN::UL_AddReconfTransChInfoList *toAsnUL_AddReconfTransChInfoList(RrcMasterChConfig *masterConfig)
{
	...
	for (TrChId tcid = 0; tcid < numtc; tcid++) {
		...
		ASN::UL_AddReconfTransChInformation *addTcIE = RN_CALLOC(ASN::UL_AddReconfTransChInformation);
...
static ASN::DL_AddReconfTransChInfoList *toAsnDL_AddReconfTransChInfoList(RrcMasterChConfig *masterConfig)
{
	...
	for (TrChId tcid = 0; tcid < numtc; tcid++) {
		...
		ASN::DL_AddReconfTransChInformation *addTcIE = RN_CALLOC(ASN::DL_AddReconfTransChInformation);
		...
		addTcIE->tfs_SignallingMode.present =
			ASN::DL_AddReconfTransChInformation__tfs_SignallingMode_PR_explicit_config;
...
void toAsnDL_AddReconfTransChInfoListSameAsUl(ASN::DL_AddReconfTransChInfoList *dlchlist,
	int dltcid, // Downlink 1-based TrCh-id to configure
	int ultcid) // Uplink 1-based TrCh-id to copy to downlink trch.
{
	ASN::DL_AddReconfTransChInformation *dummyDlDCH = RN_CALLOC(ASN::DL_AddReconfTransChInformation);
	...
	dummyDlDCH->tfs_SignallingMode.present =
		ASN::DL_AddReconfTransChInformation__tfs_SignallingMode_PR_sameAsULTrCH;
	asn_long2INTEGER(
		&dummyDlDCH->tfs_SignallingMode.choice.sameAsULTrCH.ul_TransportChannelType, ASN::UL_TrCH_Type_dch);
...
ByteVector *sendDirectTransfer(UEInfo *uep, ByteVector &dlpdu, const char *descr, bool psDomain)
{
	...
	ies->cn_DomainIdentity = psDomain ? toAsnEnumerated(ASN::CN_DomainIdentity_ps_domain)
					  : toAsnEnumerated(ASN::CN_DomainIdentity_cs_domain);
	...
	if (!encodeDcchMsg(uep, SRB3, &msg, *result, descr)) {
		return NULL;
	}
	return result;
}
...
void sendRrcConnectionSetup(UEInfo *uep, ASN::InitialUE_Identity *ueInitialId)
{
	...
	if (version4 | version5) {
		...
		// typedef ASN::RRCConnectionSetup::RRCConnectionSetup_u::RRCConnectionSetup__later_than_r3
		// later_than_r3_t;
		// ASN::RRCConnectionSetup::RRCConnectionSetup_u::RRCConnectionSetup__later_than_r3::RRCConnectionSetup__later_than_r3__criticalExtensions
		// *extp = &iep->criticalExtensions;

		if (version5) {
			// version 5 of the message.  A little verbose here...
			csp->choice.later_than_r3.criticalExtensions.present =
				ASN::RRCConnectionSetup__later_than_r3__criticalExtensions_PR_criticalExtensions;
			csp->choice.later_than_r3.criticalExtensions.choice.criticalExtensions.present =
				ASN::RRCConnectionSetup__later_than_r3__criticalExtensions__criticalExtensions_PR_r5;
			ASN::RRCConnectionSetup_r5_IEs_t *ie5 =
				&csp->choice.later_than_r3.criticalExtensions.choice.criticalExtensions.choice.r5
					 .rrcConnectionSetup_r5;
			...
			ie5->specificationMode.present =
				ASN::RRCConnectionSetup_r5_IEs__specificationMode_PR_preconfiguration;
			ie5->specificationMode.choice.preconfiguration.preConfigMode.present = ASN::
				RRCConnectionSetup_r5_IEs__specificationMode__preconfiguration__preConfigMode_PR_defaultConfig;
			asn_long2INTEGER(&ie5->specificationMode.choice.preconfiguration.preConfigMode.choice
						  .defaultConfig.defaultConfigMode,
				ASN::DefaultConfigMode_fdd);
			ie5->specificationMode.choice.preconfiguration.preConfigMode.choice.defaultConfig
				.defaultConfigIdentity = 0;

		} else {
			// version 4 of the message.
			csp->choice.later_than_r3.criticalExtensions.present =
				ASN::RRCConnectionSetup__later_than_r3__criticalExtensions_PR_r4;
			ASN::RRCConnectionSetup_r4_IEs_t *ie4 =
				&csp->choice.later_than_r3.criticalExtensions.choice.r4.rrcConnectionSetup_r4;
			// ActivationTime_t    *activationTime /* OPTIONAL */;
			// U_RNTI_t     new_U_RNTI;
			...
			if (0) {
				...
				// But this is just not right, the DL_CommonTransChInfo IEs dont even have an option of
				// setting up a FACH TrCh config. struct UL_CommonTransChInfo_r4  *ul_CommonTransChInfo
				// /* OPTIONAL */;
				...
				ie4->ul_CommonTransChInfo->modeSpecificInfo->present =
					ASN::UL_CommonTransChInfo_r4__modeSpecificInfo_PR_fdd;
				gRrcDcchConfig->getUlTfcs()->toAsnTfcs(
					&ie4->ul_CommonTransChInfo->modeSpecificInfo->choice.fdd.ul_TFCS,
					TrChUlDCHType);
				...
				ie4->dl_CommonTransChInfo->modeSpecificInfo->present =
					ASN::DL_CommonTransChInfo_r4__modeSpecificInfo_PR_fdd;
				typedef ASN::DL_CommonTransChInfo_r4::DL_CommonTransChInfo_r4__modeSpecificInfo::
					DL_CommonTransChInfo_r4__modeSpecificInfo_u::
						DL_CommonTransChInfo_r4__modeSpecificInfo__fdd::
							DL_CommonTransChInfo_r4__modeSpecificInfo__fdd__dl_Parameters
								dl_ridiculous;
				ie4->dl_CommonTransChInfo->modeSpecificInfo->choice.fdd.dl_Parameters =
					RN_CALLOC(dl_ridiculous);
				...
				// gRrcCcchConfig->getUlTfs()->toAsnTfs((prach_SI->rach_TransportFormatSet =
				// RN_CALLOC(ASN::TransportFormatSet)));
			}
...
static void sendRrcConnectionReleaseCcch(int32_t urnti)
{
	...
	ByteVector result(1000);
	if (!encodeCcchMsg(&msg, result, descrRrcConnectionRelease, NULL, urnti)) {
		return;
	}
...
void sendRrcConnectionRelease(UEInfo *uep) //, ASN::InitialUE_Identity *ueInitialId
{
	...
	*ies->n_308 = 1;		      // must be 1..8
	...
	ByteVector result(1000);
	if (!encodeDcchMsg(uep, SRB2, &msg, result, descrRrcConnectionRelease)) {
		return;
	}
...
bool sendRadioBearerSetup(UEInfo *uep, RrcMasterChConfig *masterConfig, PhCh *phch, bool srbstoo)
{
	...
	for (rbid = 5; rbid < masterConfig->mNumRB; rbid++) {
		if (!(rb = masterConfig->getRB(rbid)) || !rb->valid()) {
			continue;
		}
		haveDataCh = true;
		break;
	...
	ByteVector result(1000);
	if (!encodeDcchMsg(uep, SRB2, &msg, result, descrRadioBearerSetup)) {
		return 1;
	}
	...
	bool finished)    // If set, all RABs released so move UE back to CELL_FACH mode.
{
	...
	if (finished) {
		...
		// the two separate TFS for use in CELL_FACH and CELL_DCH modes; this critical piece of info is not
		// in 25.331. To be safe, I would like to use different TrChs numbers for RACH/FACH and DCH, but
		...
		// EXCEPT: The SIB5 can (and currently does) have the wrong FACH TrCh numbers in it, and the UE
		// apparently does not even care! The only thing I can imagine is that the UE is pretty much ignoring
		// the TrCh ids in FACH state, implying that the spec is totally screwed up here and nobody knows how it
		// is really supposed to work.
		{
			...
			// ult->ul_TransportChannelIdentity =
			// uep->mUeDchConfig.getUlTrChInfo(0)->mTransportChannelIdentity;
...

		{
			...
			// dlt->dl_TransportChannelIdentity =
			// uep->mUeDchConfig.getDlTrChInfo(0)->mTransportChannelIdentity;
	...
	ByteVector result(1000);
	if (!encodeDcchMsg(uep, SRB2, &msg, result, descrRadioBearerRelease)) {
		return;
	}
	...
	// Prepare to receive the reply to this message:
	UeTransaction(uep, UeTransaction::ttRadioBearerRelease, rabMask, transactionId, nextState);
...
static void sendCellUpdateConfirmDcch(UEInfo *uep)
{
	...
	ASN::CellUpdateConfirm_r3_IEs_t *ies = &msg.message.choice.cellUpdateConfirm.choice.r3.cellUpdateConfirm_r3;
	unsigned transactionId = commonCellUpdateConfirm(uep, ies);

	ByteVector result(1000);
	if (!encodeDcchMsg(uep, SRB2, &msg, result, descrCellUpdateConfirm)) {
		return;
	}
...
static void sendCellUpdateConfirmCcch(UEInfo *uep)
{
	...
	ByteVector result(1000);
	if (!encodeCcchMsg(&msg, result, descrCellUpdateConfirm, uep, 0)) {
		return;
	}
	UeTransaction(uep, UeTransaction::ttCellUpdateConfirm, 0, transactionId);
...
void sendSecurityModeCommand(UEInfo *uep)
{
	...
	ASN::SecurityModeCommand_r3_IEs *ies = &msg.message.choice.securityModeCommand.choice.r3.securityModeCommand_r3;
	...
	if (uep->radioCapability) {
		...
		// 8.1.12.3 implies that if these capabilities don't exactly match those of the UE's, the UE will
		// release
		...
		AsnBitString2BVTemp(ies->securityCapability.cipheringAlgorithmCap)
			.setField(ASN::SecurityCapability__cipheringAlgorithmCap_uea0, 1, 1);
		AsnBitString2BVTemp(ies->securityCapability.cipheringAlgorithmCap)
			.setField(ASN::SecurityCapability__cipheringAlgorithmCap_uea1, 1, 1);
		...
		AsnBitString2BVTemp(ies->securityCapability.integrityProtectionAlgorithmCap)
			.setField(ASN::SecurityCapability__integrityProtectionAlgorithmCap_uia1, 1,
				1); // FIXME: 10.3.3.7 of 25.331 doesn't follow this
	}
	...
	*/
	...
	ipmi->integrityProtectionModeCommand.choice.startIntegrityProtection.integrityProtInitNumber =
		allocAsnBIT_STRING(32);
	AsnBitString2BVTemp(
		ipmi->integrityProtectionModeCommand.choice.startIntegrityProtection.integrityProtInitNumber)
	...
	*/
	...
	ByteVector result(1000);
	if (!encodeDcchMsg(uep, SRB2, &msg, result, descrSecurityModeCommand)) {
		return;
	}
...
static void handleSecurityModeFailure(UEInfo *uep, ASN::SecurityModeFailure_t *secfailmsg)
{
	...
	case ASN::FailureCauseWithProtErr_PR_configurationUnsupported:
		why = "configurationUnsupported";
		break;
	case ASN::FailureCauseWithProtErr_PR_physicalChannelFailure:
		why = "physicalChannelFailure";
		break;
	case ASN::FailureCauseWithProtErr_PR_incompatibleSimultaneousReconfiguration:
		why = "incompatibleSimultaneousReconfiguration";
		break;
	case ASN::FailureCauseWithProtErr_PR_compressedModeRuntimeError:
		why = "compressedModeRuntimeError";
		break;
	case ASN::FailureCauseWithProtErr_PR_protocolError: {
		why = "protocolError";
	...
	case ASN::FailureCauseWithProtErr_PR_cellUpdateOccurred:
		why = "cellUpdateOccurred";
		break;
	case ASN::FailureCauseWithProtErr_PR_invalidConfiguration:
		why = "invalidConfiguration";
		break;
	case ASN::FailureCauseWithProtErr_PR_configurationIncomplete:
		why = "configurationIncomplete";
		break;
	case ASN::FailureCauseWithProtErr_PR_unsupportedMeasurement:
		why = "unsupportedMeasurement";
		break;
	case ASN::FailureCauseWithProtErr_PR_mbmsSessionAlreadyReceivedCorrectly:
		why = "mbmsSessionAlreadyRecievedCorrectly";
		break;
	case ASN::FailureCauseWithProtErr_PR_lowerPriorityMBMSService:
		why = "lowerPriorityMBMSService";
		break;
	default:
		break;
	}
...
static void handleCellUpdate(ASN::CellUpdate_t *msg)
{
	...
	case ASN::CellUpdateCause_radiolinkFailure:
		radioErr = true;
		break;
	case ASN::CellUpdateCause_rlc_unrecoverableError:
		otherErr = true;
		break;
	}
	...
	if (msg->failureCause) {
		...
		if (failCause == ASN::FailureCauseWithProtErr_PR_protocolError) {
			...
			asn_INTEGER2long(&msg->failureCause->failureCause.choice.protocolError.diagnosticsType.choice
						  .type1.protocolErrorCause,
				&protoErr);
		}
	}
	...
	if (!uep) {
		comment = "UL_CCCH_MessageType_PR_cellUpdate (new UE)";
	}
	...
	if (anyErr) {
		LOG(ERR) << "Received cell update message with failure info:" << LOGVAR(cuCause) << LOGVAR(srbErr)
			 << LOGVAR(rbErr) << LOGVAR(otherErr) << LOGVAR(radioErr) << LOGVAR(failCause)
			 << LOGVAR(protoErrValid) << LOGVAR(protoErr) << LOGVAR(failedTransactionId) << uep;
	} else if (uep == NULL) {
		LOG(ERR) << format("Received cell update message with unrecognized URNTI=0x%x", urnti);
	}
	...
	if (uep) {
		...
		//}
		// Dont need this printf.  The START value is not supposed to change.  It was a bug that was changing
		// it. printf("actual START value is: %u\n", uep->integrity.getStart());
...
void UEInfo::ueRecvStatusMsg(ASN::UL_DCCH_Message *msg1)
{
	...
	ASN::ProtocolErrorMoreInformation::ProtocolErrorMoreInformation__diagnosticsType::
		ProtocolErrorMoreInformation__diagnosticsType_u::ProtocolErrorMoreInformation__diagnosticsType__type1
			*info = &statusmsg->protocolErrorInformation.diagnosticsType.choice.type1;
	char const *errtype;
#define STATUS_INFO_TYPE(what) ASN::ProtocolErrorMoreInformation__diagnosticsType__type1_PR_##what : errtype = #what;
	...
	switch (info->present) {
	case STATUS_INFO_TYPE(asn1_ViolationOrEncodingError) break; case STATUS_INFO_TYPE(messageTypeNonexistent) break;
		case STATUS_INFO_TYPE(messageNotCompatibleWithReceiverState) rmsg =
			&info->choice.messageNotCompatibleWithReceiverState;
		break; case STATUS_INFO_TYPE(ie_ValueNotComprehended) rmsg = &info->choice.ie_ValueNotComprehended;
		break; case STATUS_INFO_TYPE(conditionalInformationElementError) rmsg =
			       &info->choice.conditionalInformationElementError;
		break; case STATUS_INFO_TYPE(messageExtensionNotComprehended) rmsg =
			       &info->choice.messageExtensionNotComprehended;
		break; default:
	...
	if (rmsg) {
		...
		sprintf(rbuf, " ReceivedMessageType=%ld transId=%d %s", rmsgtype, transId, tr ? tr->str().c_str() : "");
	}
	LOG(WARNING) << "Received RRCStatus errnum=" << (int)info->present << " errtype=" << errtype << this << rbuf;
}
...
void UEInfo::ueRecvDcchMessage(ByteVector &bv, unsigned rbNum)
{
	...
	case ASN::UL_DCCH_MessageType_PR_rrcConnectionSetupComplete: {
		ASN::RRCConnectionSetupComplete *rcscmsg = &msg1->message.choice.rrcConnectionSetupComplete;
		if (rcscmsg->ue_RadioAccessCapability) {
			uep->radioCapability = RN_CALLOC(ASN::UE_RadioAccessCapability);
			memcpy(uep->radioCapability, rcscmsg->ue_RadioAccessCapability,
				sizeof(struct ASN::UE_RadioAccessCapability));
		}
	...
	case ASN::UL_DCCH_MessageType_PR_rrcConnectionReleaseComplete: {
		ASN::RRCConnectionReleaseComplete *rcrcmsg = &msg1->message.choice.rrcConnectionReleaseComplete;
	...
	case ASN::UL_DCCH_MessageType_PR_radioBearerSetupComplete: {
		...
		ASN::RadioBearerSetupComplete *rbscmsg = &msg1->message.choice.radioBearerSetupComplete;
	...
	case ASN::UL_DCCH_MessageType_PR_radioBearerSetupFailure: {
		ASN::RadioBearerSetupFailure *rbsfmsg = &msg1->message.choice.radioBearerSetupFailure;
	...
	case ASN::UL_DCCH_MessageType_PR_radioBearerReleaseComplete: {
		ASN::RadioBearerReleaseComplete *rbrcmsg = &msg1->message.choice.radioBearerReleaseComplete;
	...
	case ASN::UL_DCCH_MessageType_PR_radioBearerReleaseFailure: {
		ASN::RadioBearerReleaseFailure *rbrfmsg = &msg1->message.choice.radioBearerReleaseFailure;
		transId = rbrfmsg->rrc_TransactionIdentifier;
		// Print out the failure cause; Just print the ASN enum value directly.
		// It is FailureCauseWIthProtErr_t
		unsigned failcode = rbrfmsg->failureCause.present;
		LOG(ERR) << "Radio Bearer Setup Failure code=" << failcode << " rb=" << rbNum << " ue:" << uep;
	...
	case ASN::UL_DCCH_MessageType_PR_signallingConnectionReleaseIndication: {
		...
		// Harvind (3-17-13).  This message is the industry standard for terminating a DCH connection to save
		// battery life
		...
		case stCELL_DCH:
			...
			if (lastTrans->mTransactionType == ttRadioBearerRelease && lastTrans->elapsed() < 1000) {
...
bool AsnUeId::eql(AsnUeId &other)
{
	...
	if (idType != other.idType)
		return false;
	if (mImsi != other.mImsi || mImei != other.mImei || mTmsiDS41 != other.mTmsiDS41)
		return false;
	if (mMcc != other.mMcc || mMnc != other.mMnc)
		return false;
	if (mTmsi != other.mTmsi || mPtmsi != other.mPtmsi || mEsn != other.mEsn)
		return false;
	if (mLac != other.mLac || mRac != other.mRac)
		return false;
	return true;
}
...
bool AsnUeId::RaiMatches()
{
	...
	if (mMcc != mcc && mMnc != mnc) {
		LOG(INFO) << "RAI of UE" << LOGVAR(mcc) << LOGVAR(mnc) << " does not match" << LOGVAR(mMcc)
			  << LOGVAR(mMnc);
		return false;
	}
	...
	if (lac != mLac || rac != mRac) {
		LOG(INFO) << "RAI of UE" << LOGVAR(lac) << LOGVAR(rac) << " does not match" << LOGVAR(mLac)
			  << LOGVAR(mRac);
		return false;
	}
