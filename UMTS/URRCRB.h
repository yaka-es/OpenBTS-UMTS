...
#define ASN_ENUM(basename, minval, maxval) AsnEnumMap s##basename(ASN::asn_DEF_##basename, ASN::basename##_##maxval)
#else
#define ASN_ENUM(basename, minval, maxval) extern AsnEnumMap s##basename
#endif
...
enum URlcMode { URlcModeTm, URlcModeUm, URlcModeAm };
...
struct RrcPollingInfo {			   // For RLC
	...
	void toAsnPollingInfo(ASN::PollingInfo *pi)
	{
		...
		if (mPollPdu) {
			pi->poll_PDU = sPoll_PDU.allocAsn(mPollPdu);
		}
		if (mPollSdu) {
			pi->poll_SDU = sPoll_SDU.allocAsn(mPollSdu);
		}
		pi->lastTransmissionPDU_Poll = mLastTransmissionPduPoll;
		pi->lastRetransmissionPDU_Poll = mLastRetransmissionPduPoll;
		if (mPollWindow) {
			pi->pollWindow = sPollWindow.allocAsn(mPollWindow);
		}
...
struct TransmissionRlcDiscard // 10.3.4.25
{
	...
	ASN::TransmissionRLC_Discard *toAsnTRD(ASN::TransmissionRLC_Discard *ptr)
	{
		...
		if (ptr == 0)
			ptr = RN_CALLOC(ASN::TransmissionRLC_Discard);
...
struct URlcInfo // 10.3.4.23
{
		...
		struct dlu_t { // Its a union, but C++ does not allow them.
			struct dlam_t {
				...
				UInt_z mLengthIndicatorSize; // REL-7 Length Indicator: 7 or 15.
							     // REL-6: LISize only applicable to UM, AM specifies based
							     // on pdusize NOTE: The UM uplink LISize is different - see
							     // above.
	...
	void transmissionWindowSize(unsigned val) { mul.u.AM.mTransmissionWindowSize = val; }
	...
	void missingPDU_Indicator(bool val) { mdl.u.AM.mDownlinkRlcStatusInfo.mMissingPduIndicator = val; }
...
struct RBInfo : public URlcInfo, public PdcpInfo, public virtual RrcDefs {
	...
	void setTransportChannelIdentity(unsigned tcid1based)
	{ // incoming is 1-based.
...
// 	unsigned mNASSyncIndicator;	// 10.3.4.12: 4 bits, and I quote: "A containter for non-access stratum
// information to be transferred transparently through utran. 	ReestablishmentTimer mReestablishmentTimer;
// // 10.3.3.30
// };
