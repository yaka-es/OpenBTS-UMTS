...
#include <TransceiverUHD/Transceiver.h> // FIXME
...
float invFilt[FILTLEN] = {0.010688, -0.021274, 0.040037, -0.06247, 0.10352, -0.15486, 0.23984, -0.42166, 0.97118,
	-0.42166, 0.23984, -0.15486, 0.10352, -0.06247, 0.040037, -0.021274, 0.010688};
float invFiltRcv[FILTLEN] = {0.017157, -0.028391, 0.029818, -0.043222, 0.047142, -0.063790, 0.075942, -0.136568,
	0.964300, -0.136568, 0.075942, -0.063790, 0.047142, -0.043222, 0.029818, -0.028391, 0.017157};
signalVector *inverseCICFilter;
signalVector *rcvInverseCICFilter;
signalVector *txHistoryVector;
signalVector *rxHistoryVector;

RadioModem::RadioModem(UDPSocket &wDataSocket) : mDataSocket(wDataSocket)
{
	sigProcLibSetup(1);
	...
	mUplinkPRACHScramblingCodeIndex =
		mDownlinkScramblingCodeIndex + gConfig.getNum("UMTS.PRACH.ScramblingCode"); // 4.3.3.2 of 25.213
...
void *DCHLoopAdapter(DCHLoopInfo *dli)
{
	...
	while (1) {
		...
		if (modem->gActiveDPDCH.find((void *)currDCH) == modem->gActiveDPDCH.end())
			continue;
		...
		currDPDCH->active = modem->decodeDCH(*burstCopy, wTime, uplinkScramblingCodeIndex, numPilots,
			currDPDCH->descrambledBurst, currDPDCH->rawBurst, currDPDCH->lastTOA, currDPDCH->bestTOA,
			currDPDCH->bestChannel, currDPDCH->bestSNR, currDPDCH->tfciBits, currDPDCH->tpcBits);
		...
		if (slotIx == gFrameSlots - 1) { // gots a frame, let's decode it
			...
			LOG(NOTICE) << "numTFCI: " << currDCH->l1ul()->mNumTfc << " TFCI: " << TFCI
				    << ", SF: " << (1 << uplinkSpreadingFactorLog2)
				    << ", scram: " << uplinkScramblingCodeIndex
				    << ", code: " << uplinkSpreadingCodeIndex << ", time:" << wTime;
			// LOG(INFO) << "TPC: " << currDPDCH->tpcBits[0] << " " << currDPDCH->tpcBits[1];

			if (TFCI != 0) {
				currDCH->l1ul()->mReceived = true;
				modem->decodeDPDCHFrame(*currDPDCH, uplinkScramblingCodeIndex,
					uplinkSpreadingFactorLog2, uplinkSpreadingCodeIndex);
			}
		}
		delete (signalVector *)(q->burst);
		delete q;
...
void RadioModem::generateRACHMessagePilots(int filtLen)
{
	...
	for (unsigned slot = 0; slot < gFrameSlots; slot++) {
		...
		spreadOneBranch((BitVector &)gRACHMessagePilots[slot],
			(int8_t *)gOVSFTree.code(
				mRACHMessageControlSpreadingFactorLog2, mRACHMessageControlSpreadingCodeIndex),
			(1 << mRACHMessageControlSpreadingFactorLog2), (radioData_t *)pilotSeqQ,
			(1 << mRACHMessageControlSpreadingFactorLog2));

		scramble(zeroIBurst, pilotSeqQ, pilotSeqLen, mRACHMessageAlignedScramblingCodeI + gSlotLen * slot,
			mRACHMessageAlignedScramblingCodeQ + gSlotLen * slot, pilotSeqLen, &IBurst, &QBurst);
...
signalVector *RadioModem::UplinkPilotWaveforms(int scramblingCode, int codeIndex, int numPilots, int slotIx)
{
	...
	if (mUplinkPilotWaveformMap.find(hashValue) != mUplinkPilotWaveformMap.end()) {
		...
		for (unsigned slot = 0; slot < gFrameSlots; slot++) {
			...
			spreadOneBranch((BitVector &)gPilotPatterns[Np][slot], (int8_t *)gOVSFTree.code(8, codeIndex),
			...
			scramble((radioData_t *)zeroIBurst, (radioData_t *)pilotSeq, numPilots * 256,
				scramI + gSlotLen * slot, scramQ + gSlotLen * slot, numPilots * 256, &Iside, &Qside);
...
void RadioModem::generateRACHPreambleTable(int startIx, int filtLen)
{
	...
	if (!mUplinkScramblingCodes[scramblingCodeIx]) {
		...
		for (unsigned i = 0; i < gFrameLen; i++) {
			...
			mRACHMessageAlignedScramblingCodeI[i] =
				*(mUplinkScramblingCodes[scramblingCodeIx]->ICode() + alignedIx);
			mRACHMessageAlignedScramblingCodeQ[i] =
				*(mUplinkScramblingCodes[scramblingCodeIx]->QCode() + alignedIx);
		}
	}
...
void RadioModem::generateDownlinkPilotWaveforms()
{
	...
	for (unsigned slot = 0; slot < gFrameSlots; slot++) {
		...
		for (int i = 0; i < 256; i++) {
			// LOG(INFO) << "mDPWI[ " << slot << "][" << i << "] = " << (int)
			// (mDownlinkSCHWaveformsI[slot])[i];
...
float RadioModem::estimateChannel(signalVector *wBurst, signalVector *matchedFilter, unsigned maxTOA, unsigned startTOA,
	complex *channel, float *TOA)
{
	signalVector correlatedPilots(maxTOA);
	correlate(
		wBurst, matchedFilter, &correlatedPilots, CUSTOM, true, (matchedFilter->size() - 1) + startTOA, maxTOA);
	if (channel && TOA) {
		float meanPower = 1.0;
		*channel = peakDetect(correlatedPilots, TOA, &meanPower);
		...
		/*for(int i = -10; i < 10; i++) {
			...
			if (startTOA < 500.0) LOG(INFO) << correlatedPilots[floor(*TOA+i)] << " SNR " <<
		correlatedPilots[floor(*TOA+i)].abs();
		}*/
		// LOG(INFO) << "TOA: " << *TOA;
...
void RadioModem::spread(BitVector &wBurst, int8_t *code, int codeLen, radioData_t *accI, radioData_t *accQ, int accLen,
	radioData_t gain)
{
...
void RadioModem::scramble(radioData_t *wBurstI, radioData_t *wBurstQ, int len, int8_t *codeI, int8_t *codeQ,
	int codeLen, radioData_t **rBurstI, radioData_t **rBurstQ)
...
void RadioModem::scrambleRACH(radioData_t *wBurstI, int len, int8_t *codeI, int codeLen, radioData_t **rBurstI)
...
signalVector *RadioModem::despread(signalVector &wBurst, const int8_t *code, int codeLength, bool useQ)
{
...
bool RadioModem::detectRACHPreamble(signalVector &wBurst, UMTS::Time wTime, float detectionThreshold)
{
	...
	if (!(accessSlotSet1 || accessSlotSet2))
		return false;

	for (int i = 0; i < 12; i++) {
		if (!mRACHSubchannelMask[i])
			continue;
		bool validSlot = (accessSlotSet1 && (gRACHSubchannels[i][wTime.FN() % 8] * 2 == (int)wTime.TN())) ||
				 (accessSlotSet2 && ((long)(gRACHSubchannels[i][wTime.FN() % 8] * 2 % gFrameSlots) ==
							    (long)wTime.TN()));
		if (!validSlot)
			continue;
		...
		float SNR;
		for (int j = 0; j < 16; j++) {
			if (!mRACHSignatureMask[j])
				continue;
			complex channel;
			float TOA;
			SNR = estimateChannel(
				&wBurst, mRACHTable[j], mRACHSearchSize, mRACHPreambleOffset, &channel, &TOA);
			TOA -= mRACHPreambleOffset;
			if (SNR > 6)
				LOG(INFO) << "signature: " << j << " SNR: " << SNR << " TOA: " << TOA
					  << " time: " << wTime;
			if (SNR < detectionThreshold) {
				consecutiveRACH = 0;
				consecutiveRACHTOA = 0;
			}
			if (SNR > detectionThreshold) {
				if (fabs(consecutiveRACHTOA - TOA) > 2.0) {
					consecutiveRACH = 0;
					consecutiveRACHTOA = 0;
				}
				consecutiveRACH++;
				consecutiveRACHTOA = TOA;
				LOG(NOTICE) << "signature: " << j << " SNR: " << SNR << " TOA: " << TOA
					    << " time: " << wTime << "cRACH: " << consecutiveRACH;

				consecutiveRACH = 0;
				consecutiveRACHTOA = 0;
				// if so, then send an AICH preamble ASAP.  Also be on guard to detect a 10ms/20ms RACH
				// message part. Sec. 7.3 of 25.211 states that AICH preamble must be sent 7680 (3
				// slots) or 12800 (5 slots) (AICH_Transmission_Timing) after RACH is received Add AICH
				...
				TxBitsBurst *out2 = new TxBitsBurst(gAICHSignatures[j].segment(20, 12), 256,
					mAICHSpreadingCodeIndex, mAICHResponseTime + UMTS::Time(0, 1), false);
...
bool RadioModem::decodeRACHMessage(signalVector &wBurst, UMTS::Time wTime, float detectionThreshold)
{
	...
	if (wTime < mNextRACHMessageStart)
		return false;
	...
	complex channel;
	float TOA;
	float SNR = estimateChannel(&wBurst, mRACHMessagePilotWaveforms[slotIx], 40,
		mRACHPilotsOffset + mExpectedRACHTOA - 20.0, &channel, &TOA);
	...
	descramble(truncBurst, mRACHMessageAlignedScramblingCodeI + gSlotLen * slotIx,
		mRACHMessageAlignedScramblingCodeQ + gSlotLen * slotIx, &descrambledRACH);

	signalVector *despreadRACHControl = despread(descrambledRACH,
		gOVSFTree.code(mRACHMessageControlSpreadingFactorLog2, mRACHMessageControlSpreadingCodeIndex),
		(1 << mRACHMessageControlSpreadingFactorLog2), true);

	RACHTFCI[0 + 2 * slotIx] =
		-0.5 * ((*despreadRACHControl)[8].real() / (float)(1 << mRACHMessageControlSpreadingFactorLog2)) + 0.5;
	RACHTFCI[1 + 2 * slotIx] =
		-0.5 * ((*despreadRACHControl)[9].real() / (float)(1 << mRACHMessageControlSpreadingFactorLog2)) + 0.5;
	...
	for (unsigned j = 0; j < gFrameSlots; j++) {
		...
		for (unsigned i = 0; i < slotSize; i++)
			dataBits[i] =
				(-0.5) * ((*despreadRACHData)[i + j * slotSize].real() / (float)(1 << sfLog2)) + 0.5;
		...
		slotTime.incTN(j);
		if (mNextRACHMessageStart.FN() % 2)
			slotTime.decTN(gFrameSlots); // FIXME!!
...
bool RadioModem::decodeDCH(signalVector &wBurst, UMTS::Time wTime, int uplinkScramblingCodeIndex, int numPilots,
	signalVector &descrambledBurst, signalVector &rawBurst, float &guessTOA, float &bestTOA, complex &bestChannel,
	float &bestSNR, float *TFCI, float *TPC)
{
	...
	channel = channel / idealCorrelationAmplitude;
	TOA = TOA - 384;
	LOG(INFO) << "slotIx: " << slotIx << ", SNR: " << SNR << ", guessTOA: " << guessTOA << ", TOA: " << TOA << " "
		  << corrWindow << ", c: " << channel << " abs: " << channel.abs();
	...
	descramble(truncBurst, (int8_t *)mUplinkScramblingCodes[uplinkScramblingCodeIndex]->ICode() + gSlotLen * slotIx,
	...
	signalVector *despreadDCHControl = despread(descrambleResultTFCITPC, gOVSFTree.code(8, 0), (1 << 8), true);
	// LOG(INFO) << "desp stop: " << wTime;

	// if (slotIx == 14)
	//	LOG(INFO) << "despread DCH ctrl: " << *despreadDCHControl << ", " << numPilots << " pilots should be: "
	//<< gPilotPatterns[numPilots-3][slotIx];
...
bool RadioModem::decodeDPDCHFrame(
	DPDCH &frame, int uplinkScramblingCodeIndex, int uplinkSpreadingFactorLog2, int uplinkSpreadingCodeIndex)
{
	...
	// LOG(INFO) << "des start: " << wTime;
	descramble(truncBurst, (int8_t *)mUplinkScramblingCodes[uplinkScramblingCodeIndex]->ICode(),
		(int8_t *)mUplinkScramblingCodes[uplinkScramblingCodeIndex]->QCode(), &descrambleResult);

	signalVector *despreadDCHData =
		despread(descrambleResult, gOVSFTree.code(uplinkSpreadingFactorLog2, uplinkSpreadingCodeIndex),
			(1 << uplinkSpreadingFactorLog2), false);
	...
	for (unsigned j = 0; j < gFrameSlots; j++) {
		...
		RxBitsBurst *dataBurst = new RxBitsBurst(uplinkSpreadingFactorLog2, dataBits,
			UMTS::Time(frame.frameTime.FN(), j), 0 /*TOA*/, 0 /*RSSI*/);
	...
	RxBitsBurst *dataBurst = new RxBitsBurst(uplinkSpreadingFactorLog2, dataBits, numBitsFrame,
		UMTS::Time(frame.frameTime.FN(), 0), 0 /*TOA*/, 0 /*RSSI*/);
	// if (j == 0) LOG(INFO) << "rxbits: " << *(dynamic_cast<SoftVector*>(dataBurst));
	FECDispatchInfo *q = new FECDispatchInfo;
	q->fec = (void *)frame.fec;
	q->burst = dataBurst;
...
void RadioModem::receiveSlot(signalVector *wwBurst, UMTS::Time wTime)
{
	...
	int threadCtr = 0;
	for (DCHListType::const_iterator DCHItr = DCHBegin; DCHItr != DCHEnd; DCHItr++) {
...
void RadioModem::transmitSlot(UMTS::Time nowTime, bool &underrun)
{
	...
	while (TxBitsBurst *staleBurst = mTxQueue->getStaleBurst(nowTime)) {
		LOG(NOTICE) << "dumping STALE burst in UMTS Tx Queue, nowTime: " << nowTime
			    << ", burst time:" << staleBurst->time();
		delete staleBurst;
		underrun = true;
	}
	...
	while (TxBitsBurst *next = (TxBitsBurst *)mTxQueue->getCurrentBurst(nowTime)) {
		...
		if (next->DCH()) //(next->SF()!=256)
			LOG(INFO) << "time: " << nowTime << ", spreading " << next->log2SF() << ", "
				  << next->codeIndex() << ", " << next->size() << ", " << next->rightJustified() << ", "
				  << next->DCH();
		spread(*next, (int8_t *)gOVSFTree.code(next->log2SF(), next->codeIndex()), next->SF(),
			waveformI + startIx, waveformQ + startIx, gSlotLen,
			next->DCH() ? mDCHAmplitude : mCCPCHAmplitude);
	...
	while (DCHItr != DCHEnd) {
		...
		spread(TPCBits, (int8_t *)gOVSFTree.code(downlinkLog2SF, downlinkSpreadingCodeIndex),
			downlinkSpreadingFactor, waveformI + startIx, waveformQ + startIx, gSlotLen, mDCHAmplitude);

		if (receivedBursts.find(downlinkSpreadingFactor + (downlinkSpreadingCodeIndex << 16)) !=
			receivedBursts.end()) {
			DCHItr++;
			continue;
		}
		...
		spread(TFCIBits, (int8_t *)gOVSFTree.code(downlinkLog2SF, downlinkSpreadingCodeIndex),
			downlinkSpreadingFactor, waveformI + startIx, waveformQ + startIx, gSlotLen, mDCHAmplitude);
		...
		spread(radioSlotBits, (int8_t *)gOVSFTree.code(downlinkLog2SF, downlinkSpreadingCodeIndex),
			downlinkSpreadingFactor, waveformI + startIx, waveformQ + startIx, gSlotLen, mDCHAmplitude);
		DCHItr++;
	}
#endif
	...
	// NOTE: The scrambling code is aligned 256 chips into the slot, so that chip "0" falls upon the first non-zero
	// chip of the P-CCPCH.
	...
	scramble(waveformI, waveformQ, gSlotLen, mDownlinkAlignedScramblingCodeI + gSlotLen * slotIx,
		mDownlinkAlignedScramblingCodeQ + gSlotLen * slotIx, gSlotLen, &finalWaveformI, &finalWaveformQ);
