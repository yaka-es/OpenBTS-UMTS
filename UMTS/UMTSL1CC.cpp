...
bool L1CCTrChInfo::isTrivial()
{
	...
	switch (getNumTfc()) {
	case 2:
		if (getFPI(0, 1)->getNumTB() > 1) {
			return false;
		}
		// Fall through to test tfc0
	case 1:
		if (getFPI(0, 0)->getNumTB() > 1) {
			return false;
		}
		// Fall through.
	case 0:
...
void L1FecProgInfo::text(std::ostream &os) const
{
	os << LOGVARM(mTfi) << LOGVARM(mTBSz) << LOGVARM(mNumTB) << LOGVARM(mCodedSz) << LOGVARM(mCodeInBkSz)
	   << LOGVARM(mCodeFillBits) << LOGVARM(mHighSideRMSz) << LOGVARM(mLowSideRMSz) << LOGVARM(mRFSegmentSize)
	   << LOGVARM(mRFSegmentOffset) << LOGVAR2("PB", getPB()) << LOGVAR2("TTICode", getTTICode());
}
...
void L1FecProgInfo::musteql(L1FecProgInfo *b)
{
	...
	if (TESTEQL(mTBSz) TESTEQL(mNumTB) TESTEQL(mCodedSz) TESTEQL(mCodeInBkSz) TESTEQL(mCodeFillBits) TESTEQL(
		    mHighSideRMSz) TESTEQL(mLowSideRMSz) TESTEQL(mRFSegmentSize) TESTEQL(mRFSegmentOffset) 1)
		return;
	...
	unsigned wRadioFrameSz, unsigned wTBSz, unsigned wMinNumTB, unsigned wMaxNumTB, bool wTurbo)
{
	...
	if (isDownlink) {
		...
		for (TfcId tfcj = 0; tfcj < mNumTfc; tfcj++) {
			...
			// double-check:
			if (tfcj == mNumTfc - 1) {
				assert((int)fpi->mLowSideRMSz == nout);
			}
		}
	} else {			// uplink
...
void L1CCTrChInfo::fecConfigTrivial(unsigned wSF, TTICodes wTTICode, unsigned wPB, unsigned wRadioFrameSz)
{
	// CCTrCh info:
	mNumTfc = 1;
	mNumTrCh = 1;
...
static void ulEquation1(UlTrChList *ul,
	...
	int deltaNij[RrcDefs::maxTrCh][RrcDefs::maxTfc]) // output
{
	...
	for (TrChId trchi = 0; trchi < numTrCh; trchi++) {
		for (TfcId tfcj = 0; tfcj < ulTfcs->getNumTfc(); tfcj++) {
			...
			Zij[trchi][tfcj] =
				ulSumFull ? floor(((double)ulSumRMmxNmj[trchi][tfcj] * Ndataj[tfcj]) / ulSumFull) : 0;
			// printf("%u %u %f\n",ulSumRMmxNmj[trchi][tfcj]* Ndataj[tfcj], ulSumRMmxNmj[numTrCh-1][tfcj],
			// ((double)ulSumRMmxNmj[trchi][tfcj] * Ndataj[tfcj]) / ulSumRMmxNmj[numTrCh-1][tfcj]);
			...
			LOGFECINFO << format("ulEquation1[%u,%u]: ulSum=%u Ndataj=%u Zij=%u Fi=%u Nij=%u deltaNij=%u\n",
				trchi, tfcj, ulSumRMmxNmj[trchi][tfcj], Ndataj[tfcj], Zij[trchi][tfcj], Fi, Nij,
				deltaNij[trchi][tfcj]);
		}
	}
}
...
static void dlEquation1(DlTrChList *dl,
	...
	double deltaNij[RrcDefs::maxTrCh]) // output
{
	...
	for (int trchi = 0; trchi < numTrCh; trchi++) {
		...
		LOGFECINFO << format("dlEquation1 TrCh %u Ndataj=%u Nistar=Nij=%g dlSum=%g Zij=%u deltaNij=%g\n", trchi,
			Ndataj, Nistar[trchi], dlSumRMmxNmj[trchi], Zij[trchi], deltaNij[trchi]);
	}
}
...
static void fecComputeUlNdataj(UlTrChList *ul,
	...
	L1CCTrChInfo *result, int Ndataj[RrcDefs::maxTfc],
	int SF[RrcDefs::maxTfc]) // The output result.
{
	...
	for (TfcId j = 0; j < ulTfcs->mNumTfc; j++) {
		...
		// We only want the minimum value in SET1, which is NDataj, so we dont keep the rest of SET1, only
		// Ndataj. In uplink, the Ndata for spreading factor sf = gFrameLen / sf;
...
static void fecComputeCommon(TrChList *trchlist, // Either UlTrChList or DlTrChList
	...
	for (TrChId tcid = 0; tcid < numTrCh; tcid++) {
		...
		for (RrcTfc *tfc = tfcs->iterBegin(); tfc != tfcs->iterEnd(); tfc++, tfci++) {
			...
			case RrcDefs::Convolutional:
				...
				fpi->mCodedSz =
					RrcDefs::R2EncodedSize(totalsize, &fpi->mCodeInBkSz, &fpi->mCodeFillBits);
				break;
			case RrcDefs::Turbo:
				fpi->mCodedSz =
					RrcDefs::TurboEncodedSize(totalsize, &fpi->mCodeInBkSz, &fpi->mCodeFillBits);
				break;
			default:
				assert(0);
			}
			LOGFECINFO << format("fecComputeCommon(%d,%d) TB=%ux%u coded=%u\n", tcid, tfci, fpi->mNumTB,
				fpi->mTBSz, fpi->mCodedSz);
		}
	}
}
...
static void fecComputeUlTrChSizes(UlTrChList *ul, L1CCTrChInfo *result, bool isDch = true)
{
	...
	for (TrChId tcid = 0; tcid < ulNumTrCh; tcid++) {
		for (TfcId tfci = 0; tfci != ulTfcs->getNumTfc(); tfci++) {
			...
			int Ni = (Ei + Fi - 1) / Fi;
			// int Ti = Fi * Ni;
			// Post radio frame equalisation goes into the interleaver, then radio-frame-segmentation, then
			// rate-matching.
...
static void fecComputeDlTrChSizes(DlTrChList *dl, unsigned dlRadioFrameSize, L1CCTrChInfo *result)
{
	...
	// for this TrCh.
	// Astonishingly, you use the same rate-matching eplus and eminus for all TF regardless of the number of bits in
	// the frame.
	int rfoffset = 0;
	...
	for (TrChId trchi = 0; trchi < numTrCh; trchi++) {
		for (TfcId tfcj = 0; tfcj < dlTfcs->getNumTfc(); tfcj++) {
			...
			int deltaNTTIl =
				maxNTTIil[trchi] ? (int)ceil((double)RN_ABS(deltaNi) * Xi / maxNTTIil[trchi]) : 0;
			LOGFECINFO << format(
				"fecComputeDlTrChSizes(%u,%u) deltaNi=%d coded=%u deltaNTTIl=%d maxNTTIil=%d\n", trchi,
				tfcj, deltaNi, fpi->mCodedSz, deltaNTTIl, maxNTTIil[trchi]);
			if (deltaNi < 0) {
				deltaNTTIl = -deltaNTTIl;
			}				    // above * sgn(delta Ni)
...
L1TrChEncoder::L1TrChEncoder(L1CCTrCh *wParent, L1FecProgInfo *wfpi) : mParent(wParent)
/*, mFpi(wfpi)*/
{
...
L1TrChDecoder::L1TrChDecoder(L1CCTrCh *wParent, L1FecProgInfo *wfpi) : mParent(wParent), mUpstream(NULL)
{
...
void L1ControlInterface::countBadFrame() { L1FER::countBadFrame(); }
...
void L1CCTrChDownlink::l1Multiplexer(L1FecProgInfo *fpi, BitVector &frame, unsigned intraTTIFrameNum)
{
	...
	LOG_DOWNLINK << "l1Multplexer" << LOGVAR2("RFSegSize", fpi->mRFSegmentSize)
		     << LOGVAR2("RFSegOff", fpi->mRFSegmentOffset) << LOGVAR2("FN", intraTTIFrameNum);
	...
	if (chType == PCCPCHType) {
		...
		for (unsigned i = 0; i < gFrameSlots; i++) {
			...
			TxBitsBurst *out = new TxBitsBurst(
				slotBits, phch->getDlSF(), phch->getSpCode(), mNextWriteTime.slot(i), true);
		...
		for (unsigned s = 0; s < gFrameSlots; s++) {
			...
			case DPDCHType:
				...
				mRadioSlotBuf.fill(0x7f, wp, ntpc);
				wp += ntpc;
#ifndef RELEASE99
				mRadioSlotBuf.writeFieldReversed(wp, tfciCode & tfciMask, ntfci);
#endif
			...
			TxBitsBurst *out = new TxBitsBurst(
				mRadioSlotBuf, phch->getDlSF(), phch->getSpCode(), mNextWriteTime.slot(s));
			if (chType == DPDCHType)
				out->DCH(true);
			RN_MEMLOG(TxBitsBurst, out);
			phch->getRadio()->writeHighSide(out);
...
void L1CCTrChDownlink::l1WriteHighSide(const MacTbs &tbs)
{
	...
	for (TrChId tcid = 0; tcid < numTrCh; tcid++) {
		...
		for (unsigned tbn = 0; tbn < numTB; tbn++) {
			...
			if (firstblock) {
				if (blocks[tbn]->scheduled())
					mNextWriteTime = blocks[tbn]->time();
				firstblock = false;
			}
		}
	...
	// Now send the result to the radio.
	if (tfc->getTfcsIndex() > 0)
		l1PushRadioFrames(tfc->getTfcsIndex());
}
...
void L1CCTrChUplink::l1WriteLowSide(const RxBitsBurst &burst)
{
	...
	if (receivedSlotIndex == 0) {
		// Special case: if we skipped just the last slot of previous frame, add a fillerframe and send it
		// anyway.
	...
	if (burst.size() != mSlotSize) {
		LOG(ERR) << "Incoming radio slot" << LOGVAR2("size", burst.size()) << " but expected"
			 << LOGVAR(mSlotSize);
		// punt.
		mReceiveTime = UMTS::Time(0, 1); // Set to garbage.
		return;
...
void L1CCTrChUplink::l1AccumulateSlots(const SoftVector *e, const float tfcibits[2])
{
	...
	if (mDSlotAccumulatorBuf.size() != fullsize) {
		mDSlotAccumulatorBuf.resize(fullsize);
	}
...
void L1CCTrChUplink::l1SecondDeinterleaving(SoftVector &v, unsigned tfci, unsigned frameIndex)
{
	...
	if (v.size() != mHDIBuf.size()) {
		mHDIBuf.resize(v.size());
	}
...
void L1CCTrChUplink::l1Demultiplexer(SoftVector &frame, unsigned tfci, unsigned frameIndex)
{
	...
	for (unsigned tcid = 0; tcid < getNumTrCh(); tcid++) {
		...
		if (!nbits) {
			continue;
		} // No bits for this TrCh this time.
...
void L1TrChDecoder::l1RadioFrameUnsegmentation(L1FecProgInfo *fpi, const SoftVector &frame)
{
	...
	if (mDTtiIndex < numFramesPerTti - 1) {
		return;
	}
...
void L1TrChDecoder::l1Deconcatenation(L1FecProgInfo *fpi, BitVector &b)
{
	...
	for (unsigned j = 0; j < numTB; j++) {
		...
		if (gFecTestMode) {
			LOGDEBUG << "writeLowSide3" << LOGBV(b) << LOGBV(gotParity) << LOGBV(expectParity)
				 << LOGVAR(parityOK) << "\n";
		}
		if (!parityOK) {
			frameGood = false;
...
void L1CCTrChDownlink::l1WaitToSend() const
{
	...
	if (diff > 3 || diff < 0) {
		LOG(NOTICE) << "waitToSend waited " << diff << " frames" << LOGVAR(fnbefore) << LOGVAR(mPrevWriteTime)
			    << LOGVAR(fnafter);
	}
...
void L1CCTrCh::fecConfig(TrChConfig &config)
{
	...
	// you will have to manually fill in the SF, PB and tbsize to exactly match what was programmed in RRC in
	// configDchPS()
	int sf = 256;
	int pb = 16;
	int ultbsize = 340;
	int dltbsize = 340;
	bool isTurbo = false;
	L1CCTrChUplink::fecConfigForOneTrCh(
		false, sf, TTI10ms, pb, getPhCh()->getDlRadioFrameSize(), ultbsize, 0, 1, isTurbo);
	L1CCTrChUplink::l1InstantiateUplink();
	L1CCTrChDownlink::fecConfigForOneTrCh(
		true, sf, TTI10ms, pb, getPhCh()->getUlRadioFrameSize(), dltbsize, 0, 1, isTurbo);
	...
	if (!mPhCh->isRach())
		fecComputeDlTrChSizes(
			config.dl(), getPhCh()->getDlRadioFrameSize(), static_cast<L1CCTrChDownlink *>(this));
	L1CCTrChUplink::l1InstantiateUplink();
	if (!mPhCh->isRach())
		L1CCTrChDownlink::l1InstantiateDownlink();
}
		...
		for (TfcId j = 0; j < getNumTfc(); j++) {
			...
			if (!this->mEncoders[i][tfi]) {
				if (getTCI(i)->mIsTurbo) {
					// Any one of the TFC j can be used to init the encoder for TF tfi; the sizes
					// are the same in all.
...
static void testCCProgramRach(int SF, TTICodes tticode, int PB)
{
	...
	LOGDEBUG << "RACH program test:" << LOGVAR(SF) << LOGVAR(tticode) << LOGVAR(PB) << LOGVAR(radioFrameSize)
		 << LOGVAR(ulTotalSize) << LOGVAR(requestedTBSize) << LOGVAR(tbsize) << "\n";
...
static void testCCProgramFach(int SF, TTICodes tticode, int PB)
{
	...
	LOGDEBUG << "FACH program test:" << LOGVAR(SF) << LOGVAR(tticode) << LOGVAR(PB) << LOGVAR(radioFrameSize)
		 << LOGVAR(ulTotalSize) << LOGVAR(requestedTBSize) << LOGVAR(tbsize) << "\n";
	LOGDEBUG << "manual  result:" << info1.str();
	LOGDEBUG << "compute result:" << info2.str();
	info1.musteql(info2);
...
static void testCCDch(unsigned sf, unsigned pb, TTICodes tticode, unsigned tbsize, bool isTurbo)
{
	...
	// test uplink:
	{
		...
		if (ulnumtbs <= 1) { // Too complicated for multiple TBs.
			...
			// by checking the uplink RFSegmentSize.  This is most likely to happen in the smallest non-zero
			// TFC #1.
			int rf1size = info1ul.getFPI(0, 1)->mRFSegmentSize;
			int rfcsize = infocul.getFPI(0, 1)->mRFSegmentSize;
			if (rf1size == rfcsize) {
				if (!isTurbo)
					info1ul.musteql(infocul);
			} else {
				LOGDEBUG << format(
					"Not checking info1ul because SF not equal %d != %d\n", rf1size, rfcsize);
			}
		}
		delete config;
		dch->phChClose();
			...
			for (unsigned iTTI = 0; iTTI < sizeof(TTIs) / sizeof(*TTIs); iTTI++) {
				...
				for (unsigned iTBSz = 0; iTBSz < sizeof(TBSzs) / sizeof(*TBSzs); iTBSz++) {
					...
					for (unsigned ntbs = 1; ntbs < 8; ntbs++) {
						info1.fecConfigForOneTrCh(isDownlink, sf, tticode, pb, radioFrameSize,
							tbsize, 1, ntbs, isTurbo);
						os << info1.str();
					}
				}
			}
