...
void TrChConfig::configRachTrCh(int ulSF, TTICodes ulTTICode, int ulPB, int TBSize)
{
	...
	// int ulMaxTBs = (maxTBSize + ulPB) / (ulTBSize + ulPB); // each TB has an associated parity block, so need to
	// account fo that
	...
	ul()->setCTFCSize(2)->addTFC(0)->checkCTFC(0);
...
void TrChConfig::configFachTrCh(int dlSF, TTICodes dlTTICode, int dlPB, int TBSize)
{
	...
	std::string inform(format(
		"FACH SF=%d radioFrameSize=%d total=%d tbsize=%d\n", dlSF, radioFrameSize, dlTotalSize, dlTBSize));
	LOG(INFO) << inform;
	std::cout << inform << std::endl;
	...
	dl()->setCTFCSize(2)->addTFC(0)->checkCTFC(0)->addTFC(1)->checkCTFC(1)->setPower(PowerOffset(11));
}
	...
	DCHFEC *dch, TTICodes tticode, unsigned pb, bool useTurbo, unsigned ulTBSize, unsigned dlTBSize)
{
...

	{
		...
		unsigned ulMaxTBs;
		unsigned ulDecodedSize = (useTurbo ? TurboDecodedSize(ulTotalSize) : R2DecodedSize(ulTotalSize)) -
					 ulPB; // assumes single trans. block
		if (ulTBSize) {
			...
			ulMaxTBs =
				(ulDecodedSize + ulPB) /
				(ulTBSize + ulPB); // each TB has an associated parity block, so need to account fo that
			if (ulMaxTBs == 0) {
				// Back off to a tbsize that works.
				unsigned ulMaxTBSize = quantizeDchTBSize(ulDecodedSize);
				LOG(ERR) << "Internal DCH config: uplink TB size=" << ulTBSize
					 << " is too large for SF=" << dch->getPhCh()->getUlSF()
					 << " using TBsize=" << ulMaxTBSize;
				result = false;
				ulTBSize = ulMaxTBSize;
			} else if (ulMaxTBs >= maxTbPerTrCh) {
				LOG(ERR) << "Internal DCH config: with uplink TB size=" << ulTBSize
					 << " then numTBs=" << ulMaxTBs << " which exceeds maximum=" << maxTbPerTrCh
					 << " using max instead";
...
#define TEST_SF4 0
		...
		std::string informul(
			format("configDchPS: UL SF=%u RF=%ux%u=%u pb=%u turbo=%d maxprecoded=%u tb=%ux%u=%u coded=%u\n",
				dch->getPhCh()->getUlSF(), ulNumRadioFrames, ulRadioFrameSize, ulTotalSize, ulPB,
				useTurbo, ulDecodedSize, ulMaxTBs, ulTBSize, ulTotalTB,
		...
		// NO {pat 1-24-2013: Change the DCH TrCh from 1 to 2, so TrCH == 1 used for RACH/FACH and TrCh == 2
		// used for DCH. The UE remembers the TrCh programming until it goes to idle mode, so using different
		// TrCh makes the radio bearer release easier; we dont have to reprogram the TrCh, we just switch it
		// using the RBMappingInfo. }
		...
		ul()->addTFC(0)->checkCTFC(0)->addTFC(1)->checkCTFC(1);
...

	{
		...
		if (dlTBSize) {
			...
			dlMaxTBs =
				(dlDecodedSize + dlPB) /
				(dlTBSize + dlPB); // each TB has an associated parity block, so need to account fo that
			if (dlMaxTBs == 0) {
				unsigned dlMaxTBSize = quantizeDchTBSize(dlDecodedSize);
				LOG(ERR) << "Internal DCH config: downlink TB size=" << dlTBSize
					 << " is too large for SF=" << dch->getPhCh()->getDlSF()
					 << " using TBsize=" << dlMaxTBSize;
				result = false;
				dlTBSize = dlMaxTBSize;
				dlMaxTBs = 1;
			} else if (dlMaxTBs >= maxTbPerTrCh) {
				LOG(ERR) << "Internal DCH config: with uplink TB size=" << dlTBSize
					 << " then numTBs=" << dlMaxTBs << " which exceeds maximum=" << maxTbPerTrCh
					 << " using max instead";
...
#if TEST_SF4
		if (dlMaxTBs > 12)
			dlMaxTBs = 12; // SF=4 doesnt work, so reduce.
#endif

		std::string inform(
			format("configDchPS: DL SF=%u RF=%ux%u=%u pb=%u turbo=%d maxprecoded=%u tb=%ux%u=%u coded=%u\n",
				dch->getPhCh()->getDlSF(), dlNumRadioFrames, dlRadioFrameSize, dlTotalSize, dlPB,
				useTurbo, dlDecodedSize, dlMaxTBs, dlTBSize, dlTotalTB,
		...
		dltfs->setDedicatedCh()
			...
			->addTF(dlTBSize, 1); // One frame.
		// Must +1 because we start numbering from 0.
		dl()->setCTFCSize(floor(log2(dlMaxTBs + 1)) + 1)->addTFC(0)->checkCTFC(0)->addTFC(1)->checkCTFC(1);
...
void TrChConfig::defaultConfig3TrCh()
{
	...
	dl()->setCTFCSize(6) // TODO: Is this really 6?
		...
		->addTFC(2, 1, 1, 1)
		->checkCTFC(23)
		->setPower(PowerOffset(11)); // What does setpower do here?
	...
	****/
	...
	//tpcStepSize(1);	// 1dB
	// Default config needs exactly this puncturing to fit in SF=128, but we will use lower rate coding to avoid the
	need for puncturing.
...
void RrcTfc::setTfc(TrChList *chlist, TfIndex tfa, TfIndex tfb, TfIndex tfc, TfIndex tfd)
{
	...
	if (nTrCh >= 2)
		assert(tfb < chlist->getTfs(1)->getNumTf());
	if (nTrCh >= 3)
		assert(tfc < chlist->getTfs(2)->getNumTf());
	if (nTrCh >= 4)
		assert(tfd < chlist->getTfs(3)->getNumTf());
...
#define TFCS_CASE(n) \
	...
	for (unsigned i = 0; i < mNumTfc; i++) {
		...
		if (chtype == TrChUlDCHType || chtype == TrChRACHType) {
			...
			(foo->powerOffsetInformation)->gainFactorInformation.present =
				ASN::GainFactorInformation_PR_signalledGainFactors;
			(foo->powerOffsetInformation)
				->gainFactorInformation.choice.signalledGainFactors.modeSpecificInfo.present =
				ASN::SignalledGainFactors__modeSpecificInfo_PR_fdd;
			(foo->powerOffsetInformation)
				->gainFactorInformation.choice.signalledGainFactors.modeSpecificInfo.choice.fdd
				.gainFactorBetaC = 15;
			(foo->powerOffsetInformation)
				->gainFactorInformation.choice.signalledGainFactors.gainFactorBetaD = 15;
...
bool RrcTfcs::isTrivialTfcs()
{
	...
	case 2:
		tfc1 = getTFC(1);
		if (tfc1->getNumTrCh() > 1) {
			return false;
		}
		if (tfc1->getTf(0)->getNumTB() > 1) {
			return false;
		}
		// Fall through to test tfc0
	case 1:
		tfc0 = getTFC(0);
		if (tfc0->getNumTrCh() > 1) {
			return false;
		}
		if (tfc0->getTf(0)->getNumTB() > 1) {
			return false;
		}
...
void RrcSemiStaticTFInfo::toAsnSSTF(ASN::SemistaticTF_Information *result)
{
	...
	switch (mCRCSize) {
	case 0:
		asn_long2INTEGER(&result->crc_Size, ASN::CRC_Size_crc0);
		break;
	case 8:
		asn_long2INTEGER(&result->crc_Size, ASN::CRC_Size_crc8);
		break;
	case 12:
		asn_long2INTEGER(&result->crc_Size, ASN::CRC_Size_crc12);
		break;
	case 16:
		asn_long2INTEGER(&result->crc_Size, ASN::CRC_Size_crc16);
		break;
	case 24:
		asn_long2INTEGER(&result->crc_Size, ASN::CRC_Size_crc24);
		break;
	}
}
...
ASN::CommonDynamicTF_Info *RrcTfs::DynamicTFInfo::toAsnCommon1(ASN::CommonDynamicTF_Info *result, bool isDownlink)
{
	...
	if (!result) {
		...
		result->logicalChannelList.present =
			isDownlink ? ASN::LogicalChannelList_PR_allSizes : ASN::LogicalChannelList_PR_configured;
...
static void defaultRbMappingOptionToAsn(TrChInfo *tcul, TrChInfo *tcdl, RbId rbid, ASN::RB_MappingOption *rbo)
{
	...
	{
		ASN::UL_LogicalChannelMappings *ulmappings = rbo->ul_LogicalChannelMappings =
			RN_CALLOC(ASN::UL_LogicalChannelMappings);
		...
		if (tcul->mTransportChType == TrChRACHType) {
			...
			// RLC Size List in the RB mapping info is "allSizes" then all items in "LogicalChannelList" in
			// the corresponding transport channel's TFS configuration must be "configured".  If the RLC
			// Size List is "configured", then the "LogicalChannelList" must be "allSizes" or
			// "explicitList".
