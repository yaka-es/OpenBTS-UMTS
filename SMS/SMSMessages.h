...
public:
	...
	TLAddress(GSM::TypeOfNumber wType, GSM::NumberingPlan wPlan, const char *wDigits)
		: TLElement(), mType(wType), mPlan(wPlan), mDigits(wDigits)
	{
	}

	TLAddress(const char *wDigits) : TLElement(), mType(GSM::NationalNumber), mPlan(GSM::E164Plan), mDigits(wDigits)
	{
	}
...
public:
	...
	TLUserData(unsigned wDCS, const BitVector wRawData, unsigned wLength, bool wUDHI = false)
...
public:
	...
	TLMessage() : mMMS(false), mSRI(false), mRP(false) {}
	...
	virtual void text(std::ostream &os) const { os << MTI(); }
