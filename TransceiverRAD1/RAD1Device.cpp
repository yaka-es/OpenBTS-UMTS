...
bool RAD1Device::make(bool wSkipRx)
{
	...
	if (!skipRx) {
		try {
			m_uRx = rnrad1Rx::make(0, decimRate, rbf, "ezusb.ihx");
	...
	try {
		m_uTx = rnrad1Tx::make(0, decimRate * 2, rbf, "ezusb.ihx");
...
int RAD1Device::readSamples(short *buf, int len, bool *overrun, TIMESTAMP timestamp, bool *underrun, unsigned *RSSI)
{
		...
		for (int pktNum = 0; pktNum < (readLen / 512); pktNum++) {
			...
			if (lastPktTimestamp + payloadSz / 2 / sizeof(short) != pktTimestamp)
				LOG(INFO)
					...
					<< lastPktTimestamp + payloadSz / 2 / sizeof(short) << ", got " << pktTimestamp;
	...
	// LOG(INFO) << "len: " << len << ", timeStart: " << timeStart << ", dataStart: " << dataStart << ", timestamp:
	// " << timestamp;
...
bool RAD1Device::setTxFreq(double wFreq, double wAdjFreq)
{
	...
	m_uRx->writeAuxDac(2, freq_cal << 4);
	if (!tx_setFreq(wFreq + 4 * LO_OFFSET, &actFreq))
		return false;
