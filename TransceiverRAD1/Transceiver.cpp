	  ...
	  mClockSocket(wBasePort, TRXAddress, wBasePort + 100)
{
	...
	mTransmitPriorityQueueServiceLoopThread =
		new Thread(8 * 32768); ///< thread to process transmit bursts from UMTS core
...
Transceiver::~Transceiver() { mTransmitPriorityQueue.clear(); }
...
void Transceiver::pushRadioVector(UMTS::Time &nowTime)
{
	...
	while (radioVector *staleBurst = mTransmitPriorityQueue.getStaleBurst(nowTime)) {
		LOG(NOTICE) << "dumping STALE burst in TRX->USRP interface burst:" << staleBurst->time()
			    << " now:" << nowTime;
		writeClockInterface();
		delete staleBurst;
	}
...
bool Transceiver::driveTransmitPriorityQueue()
{
	...
	while (itr < newBurst.end()) {
		*itr++ = complex((int)*bufferItr, (int)*(bufferItr + 1));
		bufferItr++;
		bufferItr++;
  ...
  delete newBurst;
	...
	// LOG(INFO) "added burst - time: " << currTime << ". radio: " << mRadioInterface->getClock()->get() << ",
	// dline: " << mTransmitDeadlineClock;
	...
	if (mOn) {
		...
		while (radioClock->get() + mTransmitLatency > mTransmitDeadlineClock) {
			...
			if (mRadioInterface->isUnderrun()) {
				...
				if (mTransmitLatency > UMTS::Time(1, 0)) { // if latency is one frame or less, don't
									   // bother decreasing, it's low enough
