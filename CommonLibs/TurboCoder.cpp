...
void ViterbiTurbo::branchCandidates()
{
	...
	for (unsigned i = 0; i < mNumCands; i += 2) {
		...
		mCandidates[i].oState =
			oStateShifted | (mGeneratorTable[0] & 2) | (mGeneratorTable[rState0 & mCMask] & 1);
		mCandidates[i].iState = iState0;
		mCandidates[i].rState = rState0;
		// 1 input extension
		mCandidates[i + 1].cost = cost;
		mCandidates[i + 1].oState =
			oStateShifted | (mGeneratorTable[1] & 2) | (mGeneratorTable[rState1 & mCMask] & 1);
...
void ViterbiTurbo::getSoftCostMetrics(const float *inSample)
{
	for (unsigned i = 0; i < mNumCands; i++) {
		...
		thisCand.cost += (((thisCand.oState & 1) ? (1.0 - inSample[0]) : inSample[0]) +
...
void SoftVector::decode(ViterbiTurbo &decoder, SoftVector &target) const
{
	...
	for (unsigned i = 0; i < K - deferral; i++, ip += step, avgCount += 2) {
		...
		*op++ = ((minCost->iState >> deferral) & 1) ? (confidence + 1.0) / 2.0 : (1.0 - confidence) / 2.0;
	}
	// flush the deferral queue
	for (int i = deferral - 1; i >= 0; i--) {
		*op++ = ((minCost->iState >> i) & 1) ? (confidence + 1.0) / 2.0 : (1.0 - confidence) / 2.0;
	}
}
...
{
	static const int pv[] = {7, 3, 11, 2, 13, 2, 17, 3, 19, 2, 23, 5, 29, 2, 31, 3, 37, 2, 41, 6, 43, 3, 47, 5, 53,
		2, 59, 2, 61, 2, 67, 2, 71, 7, 73, 5, 79, 3, 83, 2, 89, 3, 97, 5, 101, 2, 103, 5, 107, 2, 109, 6, 113,
		3, 127, 3, 131, 2, 137, 3, 139, 2, 149, 2, 151, 6, 157, 5, 163, 2, 167, 5, 173, 2, 179, 2, 181, 2, 191,
		19, 193, 5, 197, 2, 199, 3, 211, 2, 223, 3, 227, 2, 229, 6, 233, 3, 239, 7, 241, 7, 251, 6, 257, 3, 0,
		0};
