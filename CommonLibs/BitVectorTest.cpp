...
void test2O4()
{
	...
	SoftVector sv2(v2);
	cout << sv2 << endl;
	for (unsigned i = 0; i < sv2.size() / 4; i++)
		sv2[random() % sv2.size()] = 0.5;
	cout << sv2 << endl;
...
const char inter1Perm[4][8] = {{0}, {0, 1}, {0, 2, 1, 3}, {0, 4, 2, 6, 1, 5, 3, 7}};
...
void testInterleavings()
{
	...
	for (int lth = 48; lth <= 4800; lth *= 10) {
		...
		cout << "Turbo Interleaver permutation(" << lth << ") "
		     << (permutationCheck(er.permutation()) ? "ok" : "fail") << endl;
...
void testTurbo()
{
	...
	for (int k = 1; k < 5; k++) {
		...
		for (int j = 0; j < 20; j++) {
			...
			SoftVector sv2(v2);
			for (unsigned i = 0; i < sv2.size() / (k * 4); i++)
				sv2[random() % sv2.size()] = 0.5;
			BitVector v3(K);
		...
		cout << "avg noise in (" << noisein / n << "), / avg noise out (" << noiseout / n
		     << ") = " << noisein / noiseout << endl;
	}
}
