...
public:
	...
	ConfigurationRecord(const std::string &wValue)
		: mValue(wValue), mNumber(strtol(wValue.c_str(), NULL, 0)), mDefined(true)
	{
	}

	ConfigurationRecord(const char *wValue)
		: mValue(std::string(wValue)), mNumber(strtol(wValue, NULL, 0)), mDefined(true)
	{
	}
...
public:
	...
	enum Scope { GLOBALLYUNIQUE = 1, GLOBALLYSAME = 2, NEIGHBORSUNIQUE = 4, NEIGHBORSSAME = 8, NODESPECIFIC = 16 };
